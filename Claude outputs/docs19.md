...
| Tech assignment | DispatchTrack `Truck` = SP code | `Invoice.Salesperson1Code` — the SP code on the header is the assigned tech (Andrew, 9/22; it is what DispatchTrack exported as Truck). `DispatchRequestedRouteCode` and `InvoiceLabor.TechnicianCode` only as fallbacks when the header has none. |
