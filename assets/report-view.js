(function () {
  ...
  function render() {
    ...
    if (!report) {
      host.innerHTML = `<div class="empty-state">No report found. <a class="text-link" href="report-builder.html">Create a report.</a></div>`;
      return;
    }
    ...
    document.getElementById("edit-report-link").href = `report-builder.html?household=${report.householdId}&asset=${report.assetId}`;
    ...
    const breakdownBody = `
      <div class="report-score-explanation"><h3>How this score was calculated</h3><p>Each applicable checkpoint receives equal weight in this starter report. A rating of 5 earns full points. Lower ratings create the deductions below, so the score can be traced back to the recorded inspection.</p></div>
      <div class="loss-list">
    ...
    const detailsBody = `
      <div class="report-detail-columns">
        <section><h3>Equipment information</h3><dl class="report-definition-list">
          ...
          <div><dt>Next maintenance</dt><dd>${ui.shortDate(report.nextDueOn)}</dd></div>
        </dl></section>
