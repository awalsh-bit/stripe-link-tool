...
export function cardReceiptsCsv(rows) {
  ...
  const lines = [["Date", "Filed by", "Email", "Merchant", "Business purpose", "Amount", "Type", "Attachment", "Filed at", "Receipt id"].join(",")];
  for (const r of rows) {
    lines.push([r.spentOn, r.filedByName, r.filedByEmail, r.merchant, r.purpose, r.amount.toFixed(2), r.amount < 0 ? "return/credit" : "purchase", r.hasPhoto ? (r.isPdf ? "pdf" : "photo") : "no", r.createdAt || "", r.id].map(esc).join(","));
  }
  return lines.join("\r\n") + "\r\n";
}
