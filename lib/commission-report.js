...
//
// v3 is the HISTORICAL 2025 layout (surfaced during backfill): the same
// fields as v2 but rotated — Invoice, Customer, Line, Product, QTY, REVENUE
// (split-adjusted share), SPLIT FACTOR, commission (ignored), SERIAL NUMBER,
// Serial Type, Serial Cost. Detected by the raw header's "SerialCode" label
// sitting at position 4 (v2 raw has it at position 1), with a data-shape
// sniff as backup. Validated against the export's own commission column:
// tier rate x col-5 revenue reproduces col 7 across the July 2025 file.
//
...
export function parseCommissionGrid(grid) {
  ...
  let currentName = "";
  let sawHeader = false;
  let format = null; // "v1" | "v2" | "v3"
  let formatSniffed = false;
  ...
  for (const row of rows) {
    ...
    if (c0 === "Invoice #") {
      ...
      // v3 = the historical 2025 layout: its raw header carries "SerialCode"
      // at position 4 (the 2026 raw header has it at position 1).
      if (cells[4] && cells[4].toLowerCase().replace(/\s+/g, "") === "serialcode") {
        format = "v3";
      } else {
        // v2 headers mention serials or Ext Price/Split (the raw Crystal
        // export carries misaligned labels, but these words only exist in
        // v2-family exports).
    ...
    if (INVOICE_RE.test(c0.toUpperCase())) {
      ...
      if (!format) {
        // No header row — sniff from data shape: v3 has numeric qty at 4
        // with a string serial at 8; v2 carries the serial at 4 with a value
        // out at index 10; v1 has numeric qty at 4 and nothing past 7.
        format = typeof row[4] === "number" && typeof row[8] === "string" && row[8] !== ""
          ? "v3"
          : (row[10] != null && row[10] !== "") || (typeof row[4] === "string" && typeof row[5] === "number")
            ? "v2" : "v1";
        warnings.push(`Header row not found — detected ${format} format from data shape.`);
      }
      // Belt-and-braces: a header that read as v2 over data shaped like the
      // 2025 layout (numeric qty at 4, string serial at 8) is really v3.
      if (!formatSniffed && format === "v2" && typeof row[4] === "number" && row[4] <= 99 && typeof row[8] === "string" && row[8] !== "") {
        format = "v3";
        warnings.push("Header matched the 2026 layout but the data matches the 2025 layout — using 2025 column positions.");
      }
      formatSniffed = true;
      if (format === "v3") {
        // 2025 layout: qty[4], revenue (split share)[5], split factor[6],
        // commission[7] (ignored), serial[8], serial type[9], cost[10].
        const factor = toNumber(row[6]);
        lines.push({
          invoice,
