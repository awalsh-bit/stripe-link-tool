import { PDFDocument, StandardFonts, rgb } from "pdf-lib";

// ---------------------------------------------------------------------------
// Field Sales commission statement PDF (commissions.html → Download / Email).
// Same recipe as the payment receipt: pdf-lib, US Letter, logo header —
// but multi-page, with the statement's sections laid out as compact tables.
// The statement object comes straight from lib/field-sales-commissions.js.
// ---------------------------------------------------------------------------

const PAGE_W = 612;
const PAGE_H = 792;
const MARGIN = 54;
const INNER_W = PAGE_W - MARGIN * 2;

const INK = rgb(0.12, 0.14, 0.2);
const MUTED = rgb(0.42, 0.45, 0.52);
const LINE = rgb(0.9, 0.91, 0.94);
const GREEN = rgb(0.13, 0.41, 0.17);
const RED = rgb(0.6, 0.11, 0.11);

const money = (n) => {
  const v = Number(n) || 0;
  const sign = v < 0 ? "-" : "";
  return `${sign}$${Math.abs(v).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
};
const pct = (n, d = 0) => `${((Number(n) || 0) * 100).toFixed(d)}%`;

// Standard Helvetica is WinAnsi-only: one stray character ("→" in the
// showroom plan note, a "″" inches mark in a Crystal product name) makes
// pdf-lib throw and the whole statement download fail. Map the usual
// suspects to ASCII and blank anything else WinAnsi can't carry.
const GLYPH_MAP = { "→": "->", "←": "<-", "↔": "<->", "″": '"', "′": "'", "≥": ">=", "≤": "<=", "≠": "!=", "−": "-" };
const WINANSI_OK = /[\n\x20-\x7E\xA0-\xFF‘’‚“”„–—†‡•…‰‹›€™ŒœŠšŽžŸƒˆ˜]/;
const winAnsiSafe = (str) => Array.from(String(str))
  .map((ch) => GLYPH_MAP[ch] ?? (WINANSI_OK.test(ch) ? ch : "?"))
  .join("");

export async function buildCommissionStatementPdf({ statement, monthLabel, logoBytes = null, generatedAt = new Date() }) {
  const doc = await PDFDocument.create();
  const font = await doc.embedFont(StandardFonts.Helvetica);
  const bold = await doc.embedFont(StandardFonts.HelveticaBold);

  let page = null;
  let y = 0;
  let pageNum = 0;

  const footer = () => {
    page.drawText(
      `Wilson AC & Appliance — ${statement.planKey === "hvac" ? "HVAC Selling Tech" : "Field Sales"} commission statement · ${statement.name} · ${monthLabel} · page ${pageNum}`,
      { x: MARGIN, y: 30, size: 8, font, color: MUTED }
    );
  };

  const newPage = () => {
    page = doc.addPage([PAGE_W, PAGE_H]);
    pageNum += 1;
    y = PAGE_H - 60;
    footer();
  };

  const need = (space) => {
    if (y - space < 60) newPage();
  };

  const text = (str, x, opts = {}) => {
    page.drawText(winAnsiSafe(str), {
      x, y,
      size: opts.size || 9.5,
      font: opts.bold ? bold : font,
      color: opts.color || INK
    });
  };
  const rightText = (str, rightEdge, opts = {}) => {
    const f = opts.bold ? bold : font;
    const size = opts.size || 9.5;
    const safe = winAnsiSafe(str);
    const w = f.widthOfTextAtSize(safe, size);
    page.drawText(safe, { x: rightEdge - w, y, size, font: f, color: opts.color || INK });
  };
  const rule = () => {
    page.drawLine({ start: { x: MARGIN, y }, end: { x: PAGE_W - MARGIN, y }, thickness: 1, color: LINE });
  };
  const clip = (str, max) => {
    const s = String(str || "");
    return s.length > max ? `${s.slice(0, max - 1)}…` : s;
  };

  // ---------------- header ----------------
  newPage();
  let logoDrawn = false;
  if (logoBytes) {
    try {
      const logo = await doc.embedPng(logoBytes);
      const logoWidth = 140;
      const logoHeight = (logo.height / logo.width) * logoWidth;
      page.drawImage(logo, { x: MARGIN, y: y - logoHeight + 14, width: logoWidth, height: logoHeight });
      logoDrawn = true;
    } catch { /* text header below */ }
  }
  if (!logoDrawn) {
    text("Wilson AC & Appliance", MARGIN, { bold: true, size: 15 });
  }
  rightText("COMMISSION STATEMENT", PAGE_W - MARGIN, { bold: true, size: 14 });
  y -= 17;
  rightText(monthLabel, PAGE_W - MARGIN, { size: 10, color: MUTED });
  y -= 30;

  text(statement.name, MARGIN, { bold: true, size: 13 });
  rightText(statement.planLabel || statement.commissionPlan || "No plan assigned", PAGE_W - MARGIN, { size: 10, color: MUTED });
  y -= 15;
  text(`Salesperson code ${statement.code}`, MARGIN, { size: 9.5, color: MUTED });
  y -= 12;
  const e = statement.eligibility;
  if (e.required != null) {
    text(
      `Eligibility: ${money(e.trailingSerialRevenue)} delivered serial revenue over the trailing ${e.months} uploaded months (requires ${money(e.required)}) — ${e.eligible ? "ELIGIBLE" : "BELOW REQUIREMENT"}`,
      MARGIN, { size: 8.5, color: e.eligible ? GREEN : RED }
    );
  } else if (statement.planKey === "hvac") {
    text("Paid on whole-job gross margin — no rolling-revenue requirement on this plan", MARGIN, { size: 8.5, color: MUTED });
  } else {
    text(
      `Trailing delivered serial revenue: ${money(e.trailingSerialRevenue)} (no eligibility requirement on this plan)`,
      MARGIN, { size: 8.5, color: MUTED }
    );
  }
  y -= 16;
  rule();
  y -= 24;

  const sectionTitle = (title, note = "") => {
    need(40);
    text(title.toUpperCase(), MARGIN, { bold: true, size: 10 });
    if (note) text(note, MARGIN + bold.widthOfTextAtSize(title.toUpperCase(), 10) + 10, { size: 8.5, color: MUTED });
    y -= 15;
  };

  // ---------------- summary ----------------
  const t = statement.totals;
  text("Total commission", MARGIN, { size: 10, color: MUTED });
  y -= 24;
  text(money(t.commission), MARGIN, { bold: true, size: 24 });
  y -= 30;

  // HVAC Selling Tech statements pay on the JOB (whole-job margin tiers) —
  // their own layout, then done.
  if (statement.planKey === "hvac") {
    const hvacSummary = [
      ["HVAC jobs", `${t.paidJobs} job(s) · ${money(t.gross)} gross · ${money(t.margin)} margin${t.gmPercent != null ? ` (${t.gmPercent.toFixed(1)}% GM)` : ""}`, money(t.jobCommission)]
    ];
    if (t.releasedCount) hvacSummary.push(["Released holds", `${t.releasedCount} job(s) from earlier months, now paid`, money(t.releasedCommission)]);
    if (t.heldCount) hvacSummary.push(["Held — unpaid invoices", `${t.heldCount} job(s) excluded until paid`, money(t.heldCommission)]);
    for (const [label, note, value] of hvacSummary) {
      need(18);
      text(label, MARGIN, { bold: true, size: 9.5 });
      text(note, MARGIN + 110, { size: 9, color: MUTED });
      rightText(value, PAGE_W - MARGIN, { bold: true, size: 9.5 });
      y -= 17;
    }
    y -= 6;
    rule();
    y -= 24;

    const H = { invoice: MARGIN, customer: MARGIN + 72, finished: MARGIN + 190, gross: MARGIN + 288, cost: MARGIN + 336, margin: MARGIN + 386, gm: MARGIN + 416, rate: MARGIN + 446, comm: PAGE_W - MARGIN };
    const jobHead = () => {
      need(16);
      text("INVOICE", H.invoice, { size: 7.5, color: MUTED });
      text("CUSTOMER", H.customer, { size: 7.5, color: MUTED });
      text("FINISHED", H.finished, { size: 7.5, color: MUTED });
      rightText("GROSS", H.gross, { size: 7.5, color: MUTED });
      rightText("COST", H.cost, { size: 7.5, color: MUTED });
      rightText("MARGIN", H.margin, { size: 7.5, color: MUTED });
      rightText("GM", H.gm, { size: 7.5, color: MUTED });
      rightText("RATE", H.rate, { size: 7.5, color: MUTED });
      rightText("COMMISSION", H.comm, { size: 7.5, color: MUTED });
      y -= 4; rule(); y -= 12;
    };
    const jobRow = (j) => {
      need(14);
      const flags = [];
      if (j.held) flags.push("UNPAID");
      if (j.released) flags.push(`RELEASED — HELD ${j.heldFrom}`);
      else if (j.heldFrom) flags.push(`HELD ${j.heldFrom}`);
      if (j.belowFloor) flags.push("BELOW 22% GM — BASE ONLY");
      if (j.noMarginData) flags.push("NO COST DATA");
      if (j.negativeMargin) flags.push("NEGATIVE MARGIN");
      if (j.overridden) flags.push("ADJUSTED");
      if (j.tier && !j.belowFloor && !j.noMarginData) flags.push(j.tier.toUpperCase());
      text(clip(j.invoice, 13), H.invoice, { size: 8.5 });
      text(clip(j.customer || j.reference || "", 22), H.customer, { size: 8.5 });
      text(j.finishDate || "", H.finished, { size: 8.5 });
      rightText(money(j.gross), H.gross, { size: 8.5 });
      rightText(money(j.cost), H.cost, { size: 8.5 });
      rightText(money(j.margin), H.margin, { size: 8.5 });
      rightText(j.gmPercent == null ? "—" : `${j.gmPercent.toFixed(1)}%`, H.gm, { size: 8.5 });
      rightText(pct(j.rate), H.rate, { size: 8.5 });
      rightText(j.held ? `(${money(j.commission)})` : money(j.commission), H.comm, { size: 8.5, bold: !j.held, color: j.held ? MUTED : INK });
      y -= 11;
      if (flags.length) { text(flags.join("  ·  "), H.customer, { size: 6.5, color: j.held ? RED : MUTED }); y -= 9; }
      y -= 2;
    };
    const jobTotal = (label, value) => { need(16); rule(); y -= 12; text(label, H.invoice, { bold: true, size: 8.5 }); rightText(money(value), H.comm, { bold: true, size: 8.5 }); y -= 18; };

    sectionTitle("HVAC Jobs", "finished this month — % of job margin by whole-job GM");
    if (statement.jobs.length) {
      jobHead();
      for (const j of statement.jobs) jobRow(j);
      jobTotal("Total — HVAC jobs", t.jobCommission);
    } else {
      need(14); text("No HVAC jobs finished this month.", MARGIN, { size: 8.5, color: MUTED }); y -= 16;
    }
    if (statement.releasedJobs.length) {
      sectionTitle("Released Holds", "held in earlier months — invoice now paid, pays this month");
      jobHead();
      for (const j of statement.releasedJobs) jobRow(j);
      jobTotal("Total — released holds", t.releasedCommission);
    }
    if (statement.stillHeldJobs.length) {
      sectionTitle("Still Unpaid — Held", "pays automatically once a balance check shows the invoice paid");
      jobHead();
      for (const j of statement.stillHeldJobs) jobRow(j);
      y -= 6;
    }
    need(60);
    y -= 4; rule(); y -= 14;
    text("Tiers (whole-job GM → % of margin $): below 22% base only · 22–30.99% 6% · 31–37.99% 8% · 38–44.99% 10% · 45%+ 12%", MARGIN, { size: 8, color: MUTED });
    y -= 12;
    text("Gross = invoice before tax. Cost = equipment, parts, labor, misc and warranty cost as booked in ePASS.", MARGIN, { size: 8, color: MUTED });
    y -= 16;
    text(
      `Generated ${new Intl.DateTimeFormat("en-US", { timeZone: "America/Chicago", month: "long", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit" }).format(generatedAt)} from the ePASS finished-orders feed. Questions? Talk to Andrew.`,
      MARGIN, { size: 8, color: MUTED }
    );
    return doc.save();
  }

  const newLabel = statement.sectionNewTitle || "New serial inventory";
  const summaryRows = [
    [newLabel, `${money(t.newRevenue)} delivered list`, money(t.newCommission)],
    ["Closeout serial inventory", `${money(t.closeoutRevenue)} delivered list`, money(t.closeoutCommission)],
    ["EPIC Protect", `${money(t.protectSell)} sell · ${pct(t.attachRate, 2)} attach · ${pct(t.protectRate)} tier`, money(t.protectCommission)],
    ["Protect bonus", t.protectBonus ? "monthly protect at threshold" : "below monthly threshold", money(t.protectBonus)]
  ];
  if (t.releasedCount) summaryRows.push(["Released holds", `${t.releasedCount} line(s) from earlier months, now paid`, money(t.releasedCommission)]);
  if (t.heldCount) summaryRows.push(["Held — unpaid invoices", `${t.heldCount} line(s) excluded until paid`, money(t.heldCommission)]);
  if (t.discountedCount) summaryRows.push(["ePASS discounts netted", `${t.discountedCount} line(s) pay on the discounted price, not list`, money(t.discountsNetted)]);
  for (const [label, note, value] of summaryRows) {
    need(18);
    text(label, MARGIN, { bold: true, size: 9.5 });
    text(note, MARGIN + 170, { size: 9, color: MUTED });
    rightText(value, PAGE_W - MARGIN, { bold: true, size: 9.5 });
    y -= 17;
  }
  y -= 6;
  rule();
  y -= 24;

  // ---------------- section tables ----------------

  // Column layout (right-aligned numerics): invoice 90, product flexible,
  // qty, list, gm, rate, commission.
  const COLS = {
    invoice: MARGIN,
    product: MARGIN + 92,
    qty: MARGIN + 268,
    list: MARGIN + 336,
    gm: MARGIN + 382,
    rate: MARGIN + 420,
    comm: PAGE_W - MARGIN
  };

  const tableHead = (kind) => {
    need(16);
    text("INVOICE", COLS.invoice, { size: 7.5, color: MUTED });
    text(kind === "protect" ? "PLAN" : "MODEL / SERIAL", COLS.product, { size: 7.5, color: MUTED });
    rightText("QTY", COLS.qty, { size: 7.5, color: MUTED });
    rightText("LIST", COLS.list + 34, { size: 7.5, color: MUTED });
    if (kind === "new") {
      rightText("GM", COLS.gm + 26, { size: 7.5, color: MUTED });
      rightText("RATE", COLS.rate + 30, { size: 7.5, color: MUTED });
    } else if (kind === "closeout" || kind === "released") {
      rightText("RATE", COLS.rate + 30, { size: 7.5, color: MUTED });
    }
    if (kind !== "protect") rightText("COMMISSION", COLS.comm, { size: 7.5, color: MUTED });
    y -= 4;
    rule();
    y -= 12;
  };

  const lineRow = (l, kind) => {
    need(14);
    const flags = [];
    if (l.held) flags.push("UNPAID");
    if (l.notSelect) flags.push("SPIFF BRAND — NOT COMMISSIONED");
    if (l.split) flags.push(l.overridden?.credit ? "100% CREDIT" : "SPLIT 1/2");
    else if (l.overridden?.credit) flags.push("100% CREDIT");
    if (l.overridden && (l.overridden.list || l.overridden.cost || l.overridden.rate)) flags.push("ADJUSTED");
    if (l.discount) flags.push(`DISC ${l.discount.amount < 0 ? "-" : ""}$${Math.abs(l.discount.amount).toFixed(2)}`);
    if (l.heldFrom && kind !== "released") flags.push(`HELD ${l.heldFrom}`);

    text(clip(l.invoice, 15), COLS.invoice, { size: 8.5 });
    const prod = l.serialNumber ? `${l.product} · ${l.serialNumber}` : l.product;
    text(clip(prod, 34), COLS.product, { size: 8.5 });
    rightText(String(l.qty), COLS.qty, { size: 8.5 });
    rightText(money(l.revenue), COLS.list + 34, { size: 8.5 });
    if (kind === "new") {
      rightText(l.gmPercent == null ? "—" : `${l.gmPercent.toFixed(1)}%`, COLS.gm + 26, { size: 8.5 });
      rightText(pct(l.rate), COLS.rate + 30, { size: 8.5 });
    } else if (kind === "closeout" || kind === "released") {
      rightText(pct(l.rate), COLS.rate + 30, { size: 8.5 });
    }
    if (kind !== "protect") {
      rightText(l.held ? `(${money(l.commission)})` : money(l.commission), COLS.comm, { size: 8.5, bold: !l.held, color: l.held ? MUTED : INK });
    }
    y -= 11;
    if (flags.length) {
      text(flags.join("  ·  "), COLS.product, { size: 6.5, color: l.held ? RED : MUTED });
      y -= 9;
    }
    y -= 2;
  };

  const totalRow = (label, value) => {
    need(16);
    rule();
    y -= 12;
    text(label, COLS.invoice, { bold: true, size: 8.5 });
    rightText(money(value), COLS.comm, { bold: true, size: 8.5 });
    y -= 18;
  };

  // Showroom statements: unmarked (spiff-brand) lines pay $0 and would bury
  // the real select-brand lines in noise — leave them off the statement and
  // summarize them in one line. Their revenue still counts toward delivered
  // list and Protect attachment, so the totals are untouched.
  const spiffLines = statement.newLines.filter((l) => l.notSelect && !l.commission);
  const visibleNewLines = statement.newLines.filter((l) => !(l.notSelect && !l.commission));
  if (visibleNewLines.length || spiffLines.length) {
    sectionTitle(newLabel, statement.sectionNewNote || "tiered % of list by line GM");
    if (visibleNewLines.length) {
      tableHead("new");
      for (const l of visibleNewLines) lineRow(l, "new");
    }
    if (spiffLines.length) {
      need(14);
      const spiffList = spiffLines.reduce((s, l) => s + (Number(l.revenue) || 0), 0);
      text(
        `${spiffLines.length} unmarked brand line${spiffLines.length === 1 ? "" : "s"} (${money(spiffList)} delivered list) not itemized — vendor spiff, not commission. Still counted in delivered list and Protect attachment.`,
        MARGIN, { size: 8, color: MUTED }
      );
      y -= 14;
    }
    if (visibleNewLines.length) totalRow(`Total — ${newLabel.toLowerCase()}`, t.newCommission);
  }

  if (statement.closeoutLines.length) {
    sectionTitle("Closeout Serial Inventory", "open box & display — flat rate");
    tableHead("closeout");
    for (const l of statement.closeoutLines) lineRow(l, "closeout");
    totalRow("Total — closeout", t.closeoutCommission);
  }

  sectionTitle("EPIC Protect");
  need(14);
  text(
    `Delivered list ${money(t.deliveredList)}  ·  Protect sell ${money(t.protectSell)}  ·  Attachment ${pct(t.attachRate, 2)}  ·  Tier ${pct(t.protectRate)} of sell  ·  Commission ${money(t.protectCommission)}  ·  Bonus ${money(t.protectBonus)}`,
    MARGIN, { size: 8.5 }
  );
  y -= 16;
  if (statement.protectLines.length) {
    tableHead("protect");
    for (const l of statement.protectLines) lineRow(l, "protect");
    y -= 6;
  }

  if (statement.releasedLines.length) {
    sectionTitle("Released Holds", "held in earlier months — invoice now paid, pays this month");
    tableHead("released");
    for (const l of statement.releasedLines) lineRow({ ...l, heldFrom: l.heldFrom }, "released");
    totalRow("Total — released holds", t.releasedCommission);
  }

  if (statement.stillHeldLines.length) {
    sectionTitle("Still Unpaid — Held", "pays automatically once a balance check shows the invoice paid");
    tableHead("closeout");
    for (const l of statement.stillHeldLines) lineRow({ ...l, held: true, rate: undefined }, "protect");
    y -= 6;
  }

  need(30);
  y -= 6;
  rule();
  y -= 16;
  text(
    `Generated ${new Intl.DateTimeFormat("en-US", { timeZone: "America/Chicago", month: "long", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit" }).format(generatedAt)} from the Sales Order Detail warehouse. Questions? Talk to Andrew.`,
    MARGIN, { size: 8, color: MUTED }
  );

  return doc.save();
}
