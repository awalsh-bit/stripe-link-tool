...
function buildRunDetail(runRow, lineRows) {
  ...
  return {
    ...
    summary: {
      ...
      totalSellPrice: roundTo(lines.reduce((sum, line) => sum + line.sellPrice, 0), 2),
      totalCommissionAmount: roundTo(lines.reduce((sum, line) => sum + line.commissionAmount, 0), 2)
    },
    salespeople
  };
}
...
export async function listCommissionRuns() {
  ...
  const result = await pool.query(`
    SELECT
      ...
      COALESCE(SUM(l.sell_price), 0) AS total_sell_price,
      COALESCE(SUM(l.commission_amount), 0) AS total_commission_amount
