...
export async function listCommissionRuns() {
  ...
  const result = await pool.query(`
    SELECT
      r.*,
      COUNT(*) FILTER (WHERE UPPER(COALESCE(l.source_classification, '')) <> 'OMIT') AS line_count,
      COUNT(*) FILTER (WHERE UPPER(COALESCE(l.source_classification, '')) = 'OMIT') AS omitted_line_count,
      COUNT(DISTINCT NULLIF(l.salesperson_code, '')) AS salesperson_count,
      COALESCE(SUM(CASE WHEN UPPER(COALESCE(l.source_classification, '')) <> 'OMIT' THEN l.sell_price ELSE 0 END), 0) AS total_sell_price,
      COALESCE(SUM(CASE WHEN UPPER(COALESCE(l.source_classification, '')) <> 'OMIT' THEN l.commission_amount ELSE 0 END), 0) AS total_commission_amount
    FROM commission_runs r
    LEFT JOIN commission_lines l ON l.run_id = r.id
    GROUP BY r.id
  ...
  return result.rows.map((row) => ({
    ...
    omittedLineCount: Number(row.omitted_line_count || 0),
    salespersonCount: Number(row.salesperson_count || 0),
    totalSellPrice: toNumber(row.total_sell_price, 2),
    totalCommissionAmount: toNumber(row.total_commission_amount, 2)
  }));
}
...
export async function updateCommissionLineClassification(lineId, sourceClassification) {
  ...
  const allowedValues = new Set(["", "ALL", "SVC", "SPECIAL", "OMIT"]);
  if (!allowedValues.has(normalizedClassification)) {
    throw new Error("Classification must be ALL, SVC, SPECIAL, or OMIT.");
  }
