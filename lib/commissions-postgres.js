...
-- Widen gm_percent for penny-revenue lines whose GM rate explodes (e.g. $0.01
-- revenue against a $375 cost is a -3,749,900% GM). Values are also clamped at
-- import; this keeps the column safe either way. No-op once already 12,4.
...
-- Repair: Protect lines on plan-based runs must carry no line-level
-- commission (Protect pays at the salesperson level from the attach tier).
-- Early builds allowed a masked line edit that double-counted; edits to
-- these lines are now rejected, and this zeroes any that slipped in.
...
//                          (new) product. Protect (Wty) pays by the monthly
//                          attach-rate tier (same chart as Field Sales), plus
//                          the $500 bonus when Protect sales top $5,000.
...
// GM% can explode on penny-revenue lines (a $0.01 sale with real cost is a
// -3,749,900% GM). Anything past +/-9999% is noise - the tiers only care
// about the 18-31% band - so clamp before storing or computing.
function clampGmPercent(value) {
  const gm = Number(value) || 0;
  return Math.max(Math.min(gm, 9999), -9999);
}
...
// Showroom and Field Sales both pay Protect from the attach-rate tier chart;
// legacy runs (no plan snapshot) keep the old Appliance-department behavior.
function planHasGroupProtect(plan, department) {
  const normalizedPlan = normalizePlan(plan);
  if (normalizedPlan) {
    return normalizedPlan === PLAN_SHOWROOM || normalizedPlan === PLAN_FIELD_SALES;
  }
  return toText(department).toLowerCase() === "appliance";
}

function getGroupWarrantyPayoutPercent(plan, department, penetrationPercent) {
  return planHasGroupProtect(plan, department)
...
function buildSalespersonGroups(lines, salespersonStatuses = [], salespersonAdjustments = [], hvacOrderSettings = [], runId = "") {
  ...
  return Array.from(byRep.values())
    ...
    .map((group) => {
      ...
      // $500 bonus when Protect sales top $5,000 for the month — anyone on a
      // Protect-paying plan (Showroom or Field Sales) earns it.
      const protectBonusAmount = (plan === PLAN_FIELD_SALES || plan === PLAN_SHOWROOM) &&
  ...
  try {
    ...
    for (let index = 0; index < lines.length; index += 1) {
      ...
      `, [
        ...
        toNumber(clampGmPercent(line.gmPercent), 4),
        plan,
...
export async function recalculateCommissionLine(lineId, mode, value) {
  ...
  // Protect lines pay at the salesperson level from the monthly attach-rate
  // tier — a commission edit here would silently double-count on top of that
  // payout. (Sell price edits are still allowed; they feed the attach rate.)
  if (mode !== "price" && isWarrantyLine(current) && planHasGroupProtect(current.salespersonPlan, current.salespersonDepartment)) {
    throw new Error("Protect pays from the monthly attach-rate tier for this salesperson — it can't be edited per line. Use a manual adjustment for exceptions.");
  }

  let sellPrice = toNumber(current.sellPrice, 2);
  let gmPercent = toNumber(current.gmPercent, 4);
  ...
  if (mode === "percent") {
    ...
    // Sell price correction (e.g. a discount line ePASS left out of the
    // export). GM% is refreshed from the serial cost, and lines on a plan
    // re-run their plan rule so a changed margin re-tiers honestly.
    sellPrice = toNumber(value, 2);
    if (!isWarrantyLine(current) && sellPrice !== 0 && Number(current.serialCost)) {
      gmPercent = clampGmPercent(roundTo(((sellPrice - Number(current.serialCost)) / sellPrice) * 100, 4));
    }
    if (normalizePlan(current.salespersonPlan)) {
      const computed = computePlanLineCommission({ ...current, sellPrice, gmPercent }, current.salespersonPlan);
      commissionPercent = computed.percent;
      commissionAmount = computed.amount;
    } else {
      commissionAmount = roundTo(sellPrice * (Number(commissionPercent) / 100), 2);
    }
  } else {
    throw new Error("Calculation mode must be 'percent', 'amount', or 'price'.");
  }
  ...
  const updateResult = await pool.query(`
    ...
    WHERE id = $1
    RETURNING *
  `, [lineId, commissionPercent, commissionAmount, sellPrice, gmPercent]);
