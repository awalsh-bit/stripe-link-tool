...
function buildSalespersonGroups(lines, salespersonStatuses = []) {
  const byRep = new Map();
...
function buildRunDetail(runRow, lineRows, salespersonStatusRows = []) {
  const run = serializeRun(runRow);
  const lines = lineRows.map(serializeLine);
  const salespeople = buildSalespersonGroups(lines, salespersonStatusRows);
...
export async function getCommissionRunDetail(runId) {
  ...
  return buildRunDetail(runResult.rows[0], linesResult.rows, statusesResult.rows);
}
