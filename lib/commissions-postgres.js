...
function buildDefaultHvacOrderSetting(runId, salespersonKey, salesOrder, rawSetting) {
  ...
  return Array.from(byRep.values())
    .map((group) => ({
      ...
      salesOrders: Array.from(group.salesOrders.values())
        .map((order) => {
          if (!isHvacDepartment(group.salespersonDepartment)) {
            return order;
          }
          ...
          const setting = buildDefaultHvacOrderSetting(
            ...
            order.salesOrder || "",
            hvacSettingsBySalespersonAndOrder.get(`${group.salespersonKey}::${order.salesOrder || ""}`) || null
          );
    ...
    .map((group) => ({
      ...group,
      totalSellPrice: isHvacDepartment(group.salespersonDepartment)
      ...
      totalCommissionAmount: isHvacDepartment(group.salespersonDepartment)
...
export async function updateCommissionHvacOrderSettings(runId, salespersonKey, salesOrder, laborAmount, cogsAmount, overheadPercent) {
  await ensureCommissionTables();
  const pool = await getPostgresPool();
