...
const DEPOSIT_AGREEMENTS_FILE = ensureJsonFile("deposit-agreements.json", []);
const DEPOSIT_PAYMENT_EVENTS_FILE = ensureJsonFile("deposit-payment-events.json", []);
...
async function maintainServiceCardStorage() {
  ...
  const keptArchiveRows = archiveRows.filter((row) => !shouldPurgeArchivedServiceCard(row, now));
  const purgedCount = archiveRows.length - keptArchiveRows.length;
  const archiveMap = new Map(keptArchiveRows.map((row) => [row.id, row]));
  let didChange = purgedCount > 0;
  const archivedNow = [];

  for (const row of activeRows) {
    if (shouldMoveServiceCardToArchive(row, now)) {
      archiveMap.set(row.id, row);
      archivedNow.push(row);
      didChange = true;
      continue;
    }
  ...
  if (didChange) {
    ...
    // Leave a trail when the maintenance sweep moves scheduled/cancelled
    // calls (31+ days old) to the archive or purges 90+ day archive rows —
    // otherwise requests silently vanish from the queue with no record.
    // Dynamic import keeps json-only deployments (no database) working.
    if (archivedNow.length > 0 || purgedCount > 0) {
      import("./users-postgres.js")
        .then(({ recordAudit }) =>
          recordAudit({
            ip: "",
            actorUserId: null,
            ...
            targetUserId: null,
            detail: {
              archived: archivedNow.slice(0, 50).map((row) => ({
                id: row.id,
                customerName: row.customerName || "",
                erpOrderNumber: row.erpOrderNumber || "",
                queueStatus: row.queueStatus || ""
              })),
              archivedCount: archivedNow.length,
              purgedCount
            }
          })
        )
...
export async function upsertLink(record) {
  ...
  const index = nextLinks.findIndex((row) => String(row.id || "").trim() === String(record?.id || "").trim());

  if (index >= 0) {
    nextLinks[index] = record;
  } else {
    nextLinks.unshift(record);
  }
...
export async function readDepositAgreements() {
  return readJson(DEPOSIT_AGREEMENTS_FILE, []);
}

export async function writeDepositAgreements(data) {
  return writeJson(DEPOSIT_AGREEMENTS_FILE, data);
}

export async function readDepositPaymentEvents() {
  return readJson(DEPOSIT_PAYMENT_EVENTS_FILE, []);
}

export async function writeDepositPaymentEvents(data) {
  return writeJson(DEPOSIT_PAYMENT_EVENTS_FILE, data);
}
