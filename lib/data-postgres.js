...
// payment_links was created by hand in Render with
//   CHECK (workflow_type IN ('appliance', 'hvac_deposit'))
// before card capture (SetupIntent, no charge) existed, so every capture link
// died with "violates check constraint payment_links_workflow_type_check".
// Widen it once per process, idempotently, before the first write. NOT VALID
// keeps the ALTER from scanning (or tripping over) historical rows.
export const WORKFLOW_TYPES = ["appliance", "hvac_deposit", "card_capture"];
let workflowCheckPromise = null;
function ensureWorkflowTypeCheck(pool) {
  if (!workflowCheckPromise) {
    workflowCheckPromise = (async () => {
      const list = WORKFLOW_TYPES.map((t) => `'${t}'`).join(", ");
      const client = await pool.connect();
      try {
        await client.query("BEGIN");
        await client.query(`ALTER TABLE ${STORAGE_TABLE} DROP CONSTRAINT IF EXISTS ${STORAGE_TABLE}_workflow_type_check`);
        await client.query(`ALTER TABLE ${STORAGE_TABLE} ADD CONSTRAINT ${STORAGE_TABLE}_workflow_type_check CHECK (workflow_type IN (${list})) NOT VALID`);
      ...
      console.error("payment_links workflow_type check could not be widened:", err.message);
      workflowCheckPromise = null;
    });
  }
  return workflowCheckPromise;
}

async function getWritePool() {
  const pool = await getPool();
  await ensureWorkflowTypeCheck(pool);
  return pool;
}
...
function normalizeLinkRecordForStorage(record) {
  ...
  normalized.workflowType =
    normalized.workflowType === "hvac_deposit" ? "hvac_deposit" :
    normalized.workflowType === "card_capture" ? "card_capture" :
    "appliance";
  ...
  if (normalized.paidDate || Number(normalized.paidAmount) > 0) {
    ...
    // SetupIntent-capture links complete WITHOUT a payment — keep the
    // status (a card_saved row must never be coerced to paid or sent).
    normalized.active = false;
...
export async function upsertLink(record) {
  const pool = await getWritePool();
  const values = mapLinkRecordToParams(record);
  await pool.query(PAYMENT_LINK_UPSERT_SQL, values);
  return record;
}
