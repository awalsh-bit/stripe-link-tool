...
// Latest stop for a sales order (base-invoice match, sub-invoices like
// "S00064816-1" fold in) — the install-damage lookup borrows the delivery
// address from dispatch when we have one.
export async function findDeliveryStopByOrderRef(orderRef) {
  const base = String(orderRef || "").trim().toUpperCase().replace(/-\d+$/, "");
  if (!base) return null;
  const pool = await getReadyPool();
  const result = await pool.query(
    `SELECT * FROM delivery_stops
      WHERE upper(regexp_replace(order_ref, '-[0-9]+$', '')) = $1
      ORDER BY id DESC LIMIT 1`,
    [base]
  );
  return result.rows[0] ? mapStop(result.rows[0], { includePod: false }) : null;
}
