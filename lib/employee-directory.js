...
export async function upsertEmployeeDirectoryEntry({ code, name, email, department }, actorUserId = null) {
  const normalizedCode = normalizeEmployeeCode(code);
  const pool = await getReadyPool();
  const result = await pool.query(
    `INSERT INTO employee_directory (code, name, email, department, updated_at, updated_by)
