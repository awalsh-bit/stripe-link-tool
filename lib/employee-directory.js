...
export async function upsertEmployeeDirectoryEntry({ code, name, email, department, commuteMiles = 0 }, actorUserId = null) {
  const normalizedCode = normalizeEmployeeCode(code);
  const pool = await getReadyPool();
  const result = await pool.query(
    `INSERT INTO employee_directory (code, name, email, department, commute_miles, updated_at, updated_by)
