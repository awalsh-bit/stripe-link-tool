...
async function getReadyPool() {
  ...
  if (!ensurePromise) {
    ensurePromise = (async () => {
      ...
      // Job code E25 = HVAC Selling Technician (Andrew, 2026-09-23): the
      // title carrying that code pays under the HVAC plan unless an
      // executive has already pointed it somewhere else. Keyed on the CODE,
      // so relabeling the title never breaks the statements.
      await pool.query(
        `UPDATE job_titles SET commission_plan = 'HVAC Selling Technician'
          WHERE UPPER(COALESCE(code, '')) = 'E25' AND commission_plan = ''`
      );
