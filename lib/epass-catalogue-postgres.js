...
// LaborRate's price is `List` (odbc/LaborRate.csv discovery, 9/23): List /
// Cost / StandardCost / ActualCost, plus FlatRate, Warranty, Obsolete flags.
const PRICE_KEYS = ["List", "Rate", "Price", "SellingPrice", "Amount", "StandardRate", "DefaultRate", "LaborRate", "UnitPrice", "RegularRate", "RatePerHour", "Charge"];
function ratePrice(r) {
  ...
  for (const [k, v] of Object.entries(r || {})) if (/^list$|rate|price|amount/i.test(k) && !/code|date|stamp|flat|percent/i.test(k) && v !== "" && v != null && Number.isFinite(Number(v))) return num(v);
  return null;
}
...
export async function laborRateOptions({ q = "", brand = "", limit = 30, includeInactive = false } = {}) {
  const p = await pool();
  const b = up(brand); const needle = str(q, 60).toUpperCase();
  const params = [b, `${needle}%`]; let where = `${includeInactive ? "TRUE" : "active"} AND stem NOT SIMILAR TO '(ZN|DZ|WTY|FREIGHT|LAB-|TRIP|MISC)%'`;
  // match every word the tech typed, in any order ("pump drain" finds "WASHER DRAIN PUMP")
  for (const word of needle.split(/\s+/).filter(Boolean)) { params.push(`%${word}%`); where += ` AND (UPPER(description) LIKE $${params.length} OR UPPER(stem) LIKE $${params.length} OR UPPER(code) LIKE $${params.length})`; }
...
// What the rate book looks like right now — the field tool shows this when a
// search comes back empty, so "no match" and "nothing loaded" read differently.
export async function laborRateMeta() {
  const p = await pool();
  const r = (await p.query(`SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE active)::int AS active, COUNT(*) FILTER (WHERE description <> '')::int AS with_desc, MAX(loaded_at) AS loaded FROM epass_labor_rates`)).rows[0];
  return { total: r.total, active: r.active, withDesc: r.with_desc, loadedAt: r.loaded ? new Date(r.loaded).toISOString() : null };
}
