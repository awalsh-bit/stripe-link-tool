...
export async function feedInventoryRows() {
  const pool = await getReadyPool();
  const [onHand, models, meta] = await Promise.all([
  ...
  const m = meta.rows[0] || {};
  return { onHand: onHand.rows.map((r) => r.raw), models: models.rows.map((r) => r.raw), receivedAt: m.received_at ? new Date(m.received_at).toISOString() : null, pulledAt: m.pulled_at || "" };
}
