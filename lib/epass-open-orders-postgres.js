...
export async function feedInventoryRows() {
  const pool = await getReadyPool();
  const [onHand, models, prices, meta] = await Promise.all([
  ...
  const m = meta.rows[0] || {};
  return { onHand: onHand.rows.map((r) => r.raw), models: models.rows.map((r) => r.raw), listPrices: prices.rows.map((r) => ({ model: r.model_code, code: r.price_code, seq: r.seq, price: r.price == null ? null : Number(r.price) })), receivedAt: m.received_at ? new Date(m.received_at).toISOString() : null, pulledAt: m.pulled_at || "" };
}
