...
export async function replaceEpassOpenService(bundle, { filename = "" } = {}) {
  ...
  try {
    ...
    const poRows = rowsOf(bundle, "open-service-po-items");
    if (Array.isArray(bundle.datasets?.["open-service-po-items"])) {
    ...
    const counts = { tickets: seen.size, labor: labor.length, items: items.length, comments: comments.length, notes: notes.length, history: history.length };
