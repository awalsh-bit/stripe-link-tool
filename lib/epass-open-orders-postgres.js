...
export async function replaceEpassOpenOrders(bundle, { filename = "" } = {}) {
  ...
  try {
    ...
    const counts = { orders: seen.size, lines: lines.length, serials: serials.length, misc: misc.length, models: seenModels.size };
    const columns = { "open-orders": columnsOf(orders), "open-order-lines": columnsOf(lines), "open-order-serials": columnsOf(serials), "open-order-misc": columnsOf(misc), "open-order-models": columnsOf(modelRows) };
    await client.query(
...
export async function getEpassOpenOrder(code) {
  ...
  const [lines, serials, misc] = await Promise.all([
  ...
  return { ...order, lines: lines.rows.map((r) => r.raw), serials: serials.rows.map((r) => r.raw), misc: misc.rows.map((r) => r.raw) };
}
