...
export const COMMISSION_PAGE_PLANS = ["Field Sales Consultant", "Showroom Consultant"];

export function planForCommissionPlan(commissionPlan) {
  return String(commissionPlan || "") === "Showroom Consultant" ? SHOWROOM_PLAN : FIELD_SALES_PLAN;
}
...
export function computeFieldSalesStatements({ monthLines, month = null, balanceChecks = {}, trailingByCode = {}, directory = [], properNames = {}, overrides = {}, plan = FIELD_SALES_PLAN }) {
  ...
  const newLineRate = (repPlan, ov, gm) => {
    ...
    return {
      rate: tierRate,
      notSelect: false,
      belowFloor: gm < repPlan.newSerialTiers[repPlan.newSerialTiers.length - 1].minGm,
      noMarginData: false
    };
  };
