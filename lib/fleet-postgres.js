...
//
// NEXT TIME (Andrew, 2026-09-17): tie fuel receipts to actual usage.
//   card_receipts already carries the fuel purchases (merchant, amount,
//   spent_on, filed_by) and fleet_fuel_months carries Samsara's per-truck
//   gallons / fuel_cost / distance / mpg. Match receipts to a truck (driver
//   assignment on spent_on, or a vehicle picker on the receipt) and show
//   purchased $ next to Samsara-measured fuel for the same month, so a truck
//   whose receipts run well ahead of its burn stands out.
// ---------------------------------------------------------------------------
...
ALTER TABLE fleet_renewals ADD COLUMN IF NOT EXISTS document_photo_id INTEGER;
ALTER TABLE fleet_renewals ADD COLUMN IF NOT EXISTS document_name TEXT NOT NULL DEFAULT '';
...
export async function fleetOverview({ today = new Date().toISOString().slice(0, 10) } = {}) {
  ...
  for (const v of vehicles) {
    ...
    const vRenewals = renewals.rows.filter((r) => r.vehicle_id === v.id).map((r) => { const days = Math.round((new Date(dateOut(r.due_on) + "T12:00:00Z") - new Date(today + "T12:00:00Z")) / 86400000); return { id: r.id, kind: r.kind, dueOn: dateOut(r.due_on), notes: r.notes, daysLeft: days, state: days < 0 ? "overdue" : days <= 30 ? "soon" : "ok", documentId: r.document_photo_id || null, documentName: r.document_name || "" }; });
...
// Renewal documents (registration card, insurance certificate, inspection
// report …) live in the same photo store as shop invoices: report_ref
// `fleetr:<renewal id>`, kind `fleet_renewal`. Images or PDFs; the original
// filename is kept on the renewal row so the link reads sensibly.
export const RENEWAL_DOC_TYPES = new Set(["application/pdf"]);
export function isRenewalDocType(ct) { const t = String(ct || "").toLowerCase(); return t.startsWith("image/") || RENEWAL_DOC_TYPES.has(t); }

export async function saveRenewal({ id = null, vehicleId, kind, dueOn, notes = "", document = null }) {
  const pool = await getReadyPool();
  ...
  else await pool.query(`INSERT INTO fleet_renewals (id, vehicle_id, kind, due_on, notes) VALUES ($1,$2,$3,$4,$5)`, [rid, vehicleId, k, dueOn, String(notes).slice(0, 300)]);
  if (document?.buffer?.length) await setRenewalDocument(vehicleId, rid, document);
  return rid;
}
export async function setRenewalDocument(vehicleId, renewalId, document) {
  const pool = await getReadyPool();
  if (!document?.buffer?.length) throw new Error("Empty document.");
  if (!isRenewalDocType(document.contentType)) throw new Error("Renewal documents must be a PDF or an image.");
  const own = await pool.query(`SELECT id FROM fleet_renewals WHERE id = $1 AND vehicle_id = $2`, [renewalId, vehicleId]);
  if (!own.rows[0]) throw new Error("Renewal not found.");
  const photoId = await saveInstallDamagePhoto({ reportRef: `fleetr:${renewalId}`, kind: "fleet_renewal", contentType: String(document.contentType).toLowerCase(), buffer: document.buffer });
  const name = String(document.name || "").replace(/[\\/:*?"<>|]+/g, " ").trim().slice(0, 120) || "document";
  await pool.query(`UPDATE fleet_renewals SET document_photo_id = $3, document_name = $4 WHERE id = $1 AND vehicle_id = $2`, [renewalId, vehicleId, photoId, name]);
  return { documentId: photoId, documentName: name };
}
export async function removeRenewalDocument(vehicleId, renewalId) {
  const pool = await getReadyPool();
  await pool.query(`UPDATE fleet_renewals SET document_photo_id = NULL, document_name = '' WHERE id = $1 AND vehicle_id = $2`, [renewalId, vehicleId]);
}
export async function getRenewalDocument(renewalId) {
  const pool = await getReadyPool();
  const r = await pool.query(`SELECT document_photo_id, document_name FROM fleet_renewals WHERE id = $1`, [renewalId]);
  if (!r.rows[0]?.document_photo_id) return null;
  const photo = await getInstallDamagePhoto(r.rows[0].document_photo_id);
  return photo && photo.reportRef === `fleetr:${renewalId}` ? { ...photo, name: r.rows[0].document_name || "document" } : null;
}
