...
export async function saveCustomerRequestPhoto({ contentType = "image/jpeg", buffer, kind = "customer" }) {
  ...
  const result = await pool.query(
    ...
    [String(contentType).slice(0, 60), buffer, token, kind === "signature" ? "signature" : "customer"]
  );
...
export async function claimCustomerRequestPhotos(refs, reportRef) {
  ...
  for (const r of clean) {
    const result = await pool.query(
      `UPDATE install_damage_photos SET report_ref = $2
        WHERE id = $1 AND kind IN ('customer', 'signature') AND claim_token = $3 AND report_ref = ''
        RETURNING id, kind`,
      [r.id, ref, r.token]
    );
  ...
  pool.query(
    `DELETE FROM install_damage_photos
      WHERE kind IN ('customer', 'signature') AND report_ref = '' AND created_at < NOW() - INTERVAL '30 days'`
  ).catch(() => {});
  return claimed;
}
