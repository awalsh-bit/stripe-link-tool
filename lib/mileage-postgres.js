...
-- Effective-dated rate periods (replaces the per-year model so mid-year IRS
-- changes work). The rate for a given day is the most recent period whose
-- effective_from is on or before that day.
...
// Known IRS standard business mileage rates, by effective date. Seeded only
// for dates not already present, so admin edits are never overwritten.
const SEED_RATE_PERIODS = [
  ["2025-01-01", 0.67],
  ["2026-01-01", 0.725],
  ["2026-07-01", 0.76]  // IRS mid-year increase, effective July 1, 2026
];
...
async function getReadyPool() {
  ...
  if (!ensurePromise) {
    ensurePromise = (async () => {
      ...
      // One-time migration: fold any legacy per-year rates into periods
      // (year N -> effective N-01-01), preserving admin-entered values.
      await pool.query(
      ...
      // Seed the known IRS rates for any effective dates not already present.
      for (const [eff, rate] of SEED_RATE_PERIODS) {
        await pool.query(
          `INSERT INTO mileage_rate_periods (effective_from, rate) VALUES ($1, $2)
           ON CONFLICT (effective_from) DO NOTHING`,
          [eff, rate]
        );
      }
...
function periodDate(row) {
  const d = row.effective_from;
  return typeof d === "string" ? d.slice(0, 10) : d?.toISOString?.().slice(0, 10) || "";
}

export async function listRatePeriods() {
  const pool = await getReadyPool();
  const result = await pool.query(`SELECT * FROM mileage_rate_periods ORDER BY effective_from DESC`);
  return result.rows.map((row) => ({ effectiveFrom: periodDate(row), rate: Number(row.rate) }));
}

// The rate in effect on a given YYYY-MM-DD (most recent period on or before it).
export async function getRateEffectiveOn(dateStr) {
  const pool = await getReadyPool();
  const result = await pool.query(
    `SELECT rate FROM mileage_rate_periods WHERE effective_from <= $1::date ORDER BY effective_from DESC LIMIT 1`,
    [dateStr]
  );
  return result.rows[0] ? Number(result.rows[0].rate) : null;
}

// A monthly report falls entirely within one rate period (IRS changes land on
// month boundaries), so the first of the report's month determines its rate.
export async function getRateForReport(year, month) {
  const dateStr = `${year}-${String(month).padStart(2, "0")}-01`;
  return getRateEffectiveOn(dateStr);
}

export async function upsertRatePeriod(effectiveFrom, rate, actorUserId = null) {
  const pool = await getReadyPool();
  const result = await pool.query(
    `INSERT INTO mileage_rate_periods (effective_from, rate, updated_at, updated_by)
     VALUES ($1, $2, NOW(), $3)
     ON CONFLICT (effective_from) DO UPDATE SET rate = EXCLUDED.rate, updated_at = NOW(), updated_by = EXCLUDED.updated_by
     RETURNING *`,
    [effectiveFrom, rate, actorUserId]
  );
  return { effectiveFrom: periodDate(result.rows[0]), rate: Number(result.rows[0].rate) };
}
...
export async function listMileageReportsForReview({ status = "", year = null, month = null } = {}) {
  ...
  if (status === "paid") {
    where.push(`r.status = 'approved' AND r.paid_at IS NOT NULL`);
  } else if (status === "approved_unpaid") {
    where.push(`r.status = 'approved' AND r.paid_at IS NULL`);
  } else if (status) {
    params.push(status);
...
// Refresh the commute snapshot on an editable report (draft/denied) so a
// commute entered/changed in the directory after the report was started
// takes effect. Locked reports (submitted/approved) keep their snapshot.
export async function refreshReportCommute(reportId, commuteMiles) {
  const pool = await getReadyPool();
  await pool.query(
    `UPDATE mileage_reports
     SET commute_miles = $2, updated_at = NOW()
     WHERE id = $1 AND status IN ('draft','denied')`,
    [reportId, Math.max(Number(commuteMiles) || 0, 0)]
  );
}
...
// Mark approved reports as paid. Only transitions approved+unpaid rows.
// Returns the count actually updated.
export async function markMileageReportsPaid(reportIds, actorUserId = null) {
  ...
  const result = await pool.query(
    `UPDATE mileage_reports
     SET paid_at = NOW(), paid_by = $2, updated_at = NOW()
     WHERE id = ANY($1::uuid[]) AND status = 'approved' AND paid_at IS NULL`,
    [ids, actorUserId]
  );
  return result.rowCount;
}

// Delete a report that never entered the review flow. Only drafts can be
// deleted — a draft is created just by opening the Mileage page, so empty
// ones accumulate; entries cascade with the report.
export async function deleteMileageReport(reportId) {
  const pool = await getReadyPool();
  const result = await pool.query(
    `DELETE FROM mileage_reports WHERE id = $1 AND status = 'draft' RETURNING id, user_id, year, month`,
    [reportId]
  );
  return result.rows[0]
    ? { id: result.rows[0].id, userId: result.rows[0].user_id, year: result.rows[0].year, month: result.rows[0].month }
    : null;
}
