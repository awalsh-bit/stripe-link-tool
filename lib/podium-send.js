...
import { podiumOAuthConfigured, podiumConnected, podiumSendMessage, scheduleReopenAfterSend } from "./podium.js";
...
export async function sendCustomerText({ phone, body }) {
  ...
  if (apiToken && locationUid) {
    try {
      ...
      // Same no-closed-conversations rule as the OAuth path: this transport
      // used to skip the reopen entirely.
      const sent = await res.json().catch(() => null);
      const staticFetch = async (path, { method = "GET", body = null } = {}) => {
        const r = await fetch(`${base}${path}`, {
          method,
          headers: { "Authorization": `Bearer ${apiToken}`, ...(body ? { "Content-Type": "application/json" } : {}) },
          body: body ? JSON.stringify(body) : undefined
        });
        if (!r.ok) throw new Error(`Podium ${method} ${path} → ${r.status}`);
        return r.json().catch(() => null);
      };
      scheduleReopenAfterSend({
        phone: to,
        conversationUid: sent?.data?.conversationUid || sent?.data?.conversation?.uid || "",
        fetcher: staticFetch,
        label: "send"
      });
