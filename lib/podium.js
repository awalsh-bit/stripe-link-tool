...
// read_contacts added 2026-09-19 for the contact-profile link (Transition to
// Podium Conversation). Adding a scope means Podium must be reconnected once
// in Text Automations so the new consent is granted.
export const PODIUM_SCOPES = "read_messages write_messages read_users read_locations write_reviews read_contacts";
...
export async function reopenConversation(conversationUid, fetcher = podiumFetch) {
  return fetcher(`/v4/conversations/${encodeURIComponent(conversationUid)}`, {
    method: "PUT",
    body: { closed: false }
  });
}

// The reopen has to be PATIENT (2026-09-02 — payment-link and receipt texts
// were still ending up closed): Podium applies the auto-close asynchronously
// after it accepts the outbound message, so a single immediate reopen can
// run BEFORE the close lands and lose the race — and a brand-new contact's
// conversation may not be listable at all for a few seconds. So: check the
// conversation's closed flag and reopen it at 0s, 4s, 15s and 45s, stopping
// as soon as a check after a reopen finds it open. Fire-and-forget; the
// caller's send has already succeeded.
const REOPEN_DELAYS_MS = String(process.env.PODIUM_REOPEN_DELAYS_MS || "0,4000,15000,45000")
  .split(",").map((n) => Math.max(0, Number(n) || 0));

export function scheduleReopenAfterSend({ phone = "", conversationUid = "", fetcher = podiumFetch, label = "send" }) {
  let uid = String(conversationUid || "");
  let attempt = 0;
  let reopened = false;
  const run = async () => {
    try {
      if (!uid && phone) {
        const convo = await findConversationByPhone(phone, fetcher);
        uid = convo?.uid || "";
      }
      if (uid) {
        const current = await fetcher(`/v4/conversations/${encodeURIComponent(uid)}`);
        const closed = current?.data?.closed;
        if (closed === false && reopened) return; // verified open after our reopen — done
        if (closed !== false) {
          await reopenConversation(uid, fetcher);
          reopened = true;
        }
      }
    } catch (err) {
      console.error(`Podium reopen-after-${label} attempt ${attempt + 1} skipped:`, err.message);
    }
    attempt++;
    if (attempt < REOPEN_DELAYS_MS.length) {
      const t = setTimeout(run, REOPEN_DELAYS_MS[attempt]);
      if (typeof t.unref === "function") t.unref();
    }
  };
  const first = setTimeout(run, REOPEN_DELAYS_MS[0] || 0);
...
export async function podiumSendMessage({ phone, body }) {
  ...
  // Undo the auto-close the outbound message triggers (see above). The
  // message response names its conversation when Podium includes it;
  // otherwise the guard finds it by phone.
  scheduleReopenAfterSend({
    phone,
    conversationUid: result?.data?.conversationUid || result?.data?.conversation?.uid || "",
    label: "send"
  });
  return result;
}
...
export async function findConversationByPhone(phone, fetcher = podiumFetch) {
  ...
  for (let page = 0; page < 5; page++) {
    ...
    const data = await fetcher(`/v4/conversations${query}`);
...
// "Transition to Podium Conversation" (Andrew, 2026-09-17). Podium publishes
// no deep link, but its own CRM contact list opens a thread by handing the
// inbox an E.164 phone number — captured from a real click on a contact:
//   https://app.podium.com/inbox?assignedUser=all&locationUid=…&phoneNumber=%2B15125635219
// and Podium resolves it to /inbox/<its numeric id>. What does NOT work on an
// inbound link (all tried 2026-09-17): searchFilter/searchTerm (ignored),
// _sid=conversationUid_<uid> (tracking, not a selector), and /inbox/<id>
// (opens that one thread — the API never returns the numeric id anyway).
// On a phone with the Podium app installed the app claims the link;
// otherwise it's the web inbox. PODIUM_INBOX_URL overrides the base.
export function podiumInboxBaseUrl() {
  return String(process.env.PODIUM_INBOX_URL || "https://app.podium.com/inbox").trim().replace(/[?#].*$/, "").replace(/\/+$/, "");
}
export function buildInboxSearchUrl(phone, locationUid = "") {
  const digits = String(phone || "").replace(/\D/g, "").slice(-10);
  const params = new URLSearchParams({ assignedUser: "all" });
  if (locationUid) params.set("locationUid", String(locationUid));
  params.set("phoneNumber", `+1${digits}`);
  return `${podiumInboxBaseUrl()}?${params.toString()}`;
}
// 2026-09-19, after Andrew tested every inbox variant: the inbox routes ONLY
// on Podium's internal numeric thread id, which the API never returns —
// /inbox/<conversationUid>, ?conversationUid=, and the CRM's own params
// minus the number all fall back to the last-open thread. What Podium does
// expose is the CONTACT profile, keyed by a uid the Contacts API returns:
//   https://app.podium.com/contacts?contactUid=<uid>&locationUid=<loc>
// (captured from clicking a name in the CRM). The thread is one tap from
// there, so that is the link the button opens when the contact exists.
export function buildContactUrl(contactUid, locationUid = "") {
  const params = new URLSearchParams({ contactUid: String(contactUid) });
  if (locationUid) params.set("locationUid", String(locationUid));
  return `${podiumInboxBaseUrl().replace(/\/inbox$/, "")}/contacts?${params.toString()}`;
}
// GET /v4/contacts/{identifier} takes a phone number in E.164 form (needs
// the read_contacts scope). null when Podium has no contact for the number.
export async function findContactByPhone(phone, fetcher = podiumFetch) {
  const digits = last10(phone);
  if (digits.length !== 10) return null;
  try {
    const data = await fetcher(`/v4/contacts/${encodeURIComponent(`+1${digits}`)}`);
    const c = data?.data || data;
    if (!c || typeof c !== "object") return null;
    const uid = String(c.uid || c.contactUid || "");
    return uid ? { uid, name: c.name || "", conversationUid: c.conversationUid || "" } : null;
  } catch (err) {
    if (err.status === 404) return null;
    throw err;
  }
}
// Phone → the best link Podium allows: the contact profile when the number
// is a known contact, else the inbox with the number handed along (which at
// least lands the user in the right app). `found` tells the button which.
export async function findConversationInboxLink(phone) {
  const locationUid = await getDefaultLocationUid().catch(() => "");
  const inboxUrl = buildInboxSearchUrl(phone, locationUid);
  try {
    const contact = await findContactByPhone(phone);
    if (contact) return { url: buildContactUrl(contact.uid, locationUid), kind: "contact", found: true, contactUid: contact.uid, contactName: contact.name, conversationUid: contact.conversationUid };
    return { url: inboxUrl, kind: "inbox", found: false };
  } catch (err) {
    // 403 = the connection predates the read_contacts scope: say so once in
    // the log, fall back to the inbox link, keep working.
    if (err.status === 403) console.warn("Podium contact lookup needs the read_contacts scope — reconnect Podium in Text Automations.");
    return { url: inboxUrl, kind: "inbox", found: null, lookupError: err.message, needsReconnect: err.status === 403 };
  }
}
...
export async function addConversationNote(conversationUid, body, senderName) {
  ...
  // "handled and closed" (see reopenConversation above). Same patient guard.
  scheduleReopenAfterSend({ conversationUid, label: "note" });
  return result;
}
