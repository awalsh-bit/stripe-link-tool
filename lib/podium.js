...
// no deep link, but its web inbox selects a thread from the query string —
// captured from a real inbox URL:
//   https://app.podium.com/inbox/39701299?locationUid=…&_sid=conversationUid_<uid>
// (assignedUser was dropped on purpose: it's an inbox filter that would hide
// a thread assigned to someone else). On a phone with the Podium app
// installed the app picks the link up; otherwise it opens the web inbox.
// PODIUM_INBOX_URL overrides the inbox base if Podium's numeric id changes.
export function podiumInboxBaseUrl() {
  return String(process.env.PODIUM_INBOX_URL || "https://app.podium.com/inbox/39701299").trim().replace(/[?#].*$/, "").replace(/\/+$/, "");
}
export function buildConversationInboxUrl(conversationUid, locationUid = "") {
  const params = new URLSearchParams();
  if (locationUid) params.set("locationUid", String(locationUid));
  params.set("_sid", `conversationUid_${String(conversationUid || "").trim()}`);
  return `${podiumInboxBaseUrl()}?${params.toString()}`;
}
// Phone → the inbox link for that customer's latest Podium thread (or null
// when Podium has never talked to that number).
export async function findConversationInboxLink(phone) {
  ...
  return { conversationUid: convo.uid, contactName: convo.contactName, url: buildConversationInboxUrl(convo.uid, locationUid) };
}
