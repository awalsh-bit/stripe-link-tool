...
// no deep link, and the numeric segment in an inbox URL
// (app.podium.com/inbox/51692699) is the thread's own web id, which the API
// never returns (it only knows the uid; the _sid=conversationUid_… seen on
// CRM-sourced links is tracking, not a selector). What DOES work from the
// outside is the inbox's contact search, captured from a real URL:
//   https://app.podium.com/inbox?assignedUser=all&locationUid=…
//     &searchFilter=contacts&searchTerm=5125551234&viewAllLocations=false
// so the button opens the inbox already searched for the customer's number —
// one tap on the result lands on the thread. On a phone with the Podium app
// installed the app picks the link up; otherwise it's the web inbox.
//
// The search parameters are only honored on the /inbox/<id> route — bare
// /inbox drops them and shows the last-open thread (verified 2026-09-17: the
// same link with an id in the path landed on the right customer). The id is
// just an anchor; any real conversation id from Wilson's inbox works, and
// PODIUM_INBOX_URL overrides the whole base if Podium ever changes routes.
export function podiumInboxBaseUrl() {
  return String(process.env.PODIUM_INBOX_URL || "https://app.podium.com/inbox/51692699").trim().replace(/[?#].*$/, "").replace(/\/+$/, "");
}
export function buildInboxSearchUrl(phone, locationUid = "") {
  ...
  params.set("searchFilter", "contacts");
  params.set("searchTerm", digits);
  params.set("viewAllLocations", "false");
  return `${podiumInboxBaseUrl()}?${params.toString()}`;
}
// Phone → the inbox-search link for that number, plus whether Podium already
// has a thread with it (read-only lookup; the link is returned either way).
export async function findConversationInboxLink(phone) {
  const locationUid = await getDefaultLocationUid().catch(() => "");
  const url = buildInboxSearchUrl(phone, locationUid);
  let convo = null;
  try { convo = await findConversationByPhone(phone); } catch (err) { return { url, found: null, lookupError: err.message }; }
  return { url, found: !!convo, conversationUid: convo?.uid || "", contactName: convo?.contactName || "" };
}
