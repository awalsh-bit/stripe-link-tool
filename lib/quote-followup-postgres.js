...
-- PRECOMPUTED CONVERSIONS (Andrew, 2026-09-16). The board used to find each
-- quote's matching sales order with two LATERAL subqueries per row whose
-- WHERE was wrapped in a CASE — which Postgres can't drive from an index,
-- so every page load scanned sales_order_detail once per quote. Now the
-- match is computed ONCE, when quotes or OE-23 orders are uploaded (and
-- lazily for any row never matched), and stored on the quote row. The
-- board is a plain indexed read.
ALTER TABLE quote_followup_quotes ADD COLUMN IF NOT EXISTS conv_invoice TEXT NOT NULL DEFAULT '';
ALTER TABLE quote_followup_quotes ADD COLUMN IF NOT EXISTS conv_finish TEXT NOT NULL DEFAULT '';
ALTER TABLE quote_followup_quotes ADD COLUMN IF NOT EXISTS conv_revenue NUMERIC(14,2);
ALTER TABLE quote_followup_quotes ADD COLUMN IF NOT EXISTS other_sp TEXT NOT NULL DEFAULT '';
ALTER TABLE quote_followup_quotes ADD COLUMN IF NOT EXISTS other_invoice TEXT NOT NULL DEFAULT '';
ALTER TABLE quote_followup_quotes ADD COLUMN IF NOT EXISTS other_finish TEXT NOT NULL DEFAULT '';
ALTER TABLE quote_followup_quotes ADD COLUMN IF NOT EXISTS matched_at TIMESTAMPTZ;
CREATE INDEX IF NOT EXISTS idx_qfq_matched ON quote_followup_quotes (matched_at) WHERE matched_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_qfq_board ON quote_followup_quotes (sp_code, date_created);

-- FORCED SPLITS (Andrew, 2026-09-16). The board groups a customer's quotes
-- written within 30 days onto one opportunity card — right for a homeowner,
-- wrong for a builder or designer running several projects at once. A rep
-- can split a card: the chosen quotes get a project key (and a label such
-- as "Lot 14 — Ridgeview") and from then on group ONLY with each other,
-- regardless of date. Clearing the key puts them back under the normal rule.
CREATE TABLE IF NOT EXISTS quote_followup_projects (
  quote_number TEXT PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_qfp_key ON quote_followup_projects (project_key);
`;

// Indexes on the OE-23 warehouse that the matcher relies on. Created here
// (idempotently) because this module is the one that depends on them; the
// table itself belongs to sales-order-detail-postgres.js.
const MATCH_INDEX_SQL = `
CREATE INDEX IF NOT EXISTS idx_sod_customer_sp ON sales_order_detail (customer_number, salesperson_code, finish_date);
CREATE INDEX IF NOT EXISTS idx_sod_name_norm ON sales_order_detail (upper(trim(customer_name)), finish_date);
CREATE INDEX IF NOT EXISTS idx_sod_sp_finish ON sales_order_detail (salesperson_code, finish_date);
`;
...
// Which ePASS code did the quote export print as "Customer #"? Compare the
// feed's SoldToCode / BillToCode for each quote against the customer number
// already stored for it (from the spreadsheet uploads) and take the field
// that agrees more often. Falls back to SoldToCode with nothing to compare.
export async function calibrateQuoteCustomerField(rows) {
  const pool = await getReadyPool();
  const codes = (rows || []).map((r) => String(r?.Code || r?.code || "").trim().toUpperCase()).filter(Boolean).slice(0, 3000);
  if (!codes.length) return { field: "SoldToCode", compared: 0, sold: 0, bill: 0 };
  const stored = (await pool.query(`SELECT quote_number, customer_number FROM quote_followup_quotes WHERE quote_number = ANY($1) AND customer_number <> ''`, [codes])).rows;
  const byQ = new Map(stored.map((r) => [r.quote_number, String(r.customer_number).trim()]));
  let sold = 0, bill = 0, compared = 0;
  for (const r of rows || []) {
    const q = String(r?.Code || r?.code || "").trim().toUpperCase(); const want = byQ.get(q); if (!want) continue;
    compared += 1;
    if (String(r.SoldToCode || "").trim() === want) sold += 1;
    if (String(r.BillToCode || "").trim() === want) bill += 1;
  }
  return { field: bill > sold ? "BillToCode" : "SoldToCode", compared, sold, bill };
}
...
export async function replaceOpenQuotes(quotes, { filename, byEmail, byName }) {
  ...
  try {
    ...
    // The feed lands every 15 minutes; keep a week of its stamps, not years.
    await client.query(`DELETE FROM quote_followup_uploads WHERE uploaded_by = 'epass-agent' AND uploaded_at < NOW() - INTERVAL '7 days'`);
  ...
  // Match every board-visible row against the OE-23 warehouse now, so the
  // page never has to.
  await refreshQuoteConversions("open").catch((e) => console.error("Quote conversion refresh failed:", e.message));
...
// ---------------------------------------------------------------------------
// Recompute the cached conversion columns. scope:
//   "unmatched" — rows never matched (first deploy, or brand-new quotes)
//   "open"      — everything the board can show (open, dispositioned, QUOTE 2)
//   "all"       — every row (after an OE-23 upload, older quotes may convert)
//   [numbers]   — specific quote numbers
// Two index-driven laterals (no CASE): ePASS quotes by customer + SP code,
// service leads by normalized customer name within 30 days.
export async function refreshQuoteConversions(scope = "unmatched") {
  const pool = await getReadyPool();
  const list = Array.isArray(scope) ? scope.map((q) => String(q || "").trim()).filter(Boolean) : null;
  if (scope === "unmatched") {
    // Every board load calls this; make the nothing-to-do case one indexed probe.
    const pending = await pool.query(`SELECT 1 FROM quote_followup_quotes WHERE matched_at IS NULL LIMIT 1`);
    if (!pending.rows.length) return { updated: 0 };
  }
  try { await pool.query(MATCH_INDEX_SQL); } catch (e) { /* warehouse not created yet — nothing to match against */ return { updated: 0, skipped: e.message }; }
  // Fresh statistics right after a bulk upload: without them the planner
  // can still pick a sequential scan per quote (measured: 27 s vs 0.3 s).
  await pool.query(`ANALYZE sales_order_detail`).catch(() => {});
  await pool.query(`ANALYZE quote_followup_quotes`).catch(() => {});
  const where = list ? `q.quote_number = ANY($1)`
    : scope === "all" ? `TRUE`
    : scope === "open" ? `(q.is_open OR upper(trim(q.job_status)) = 'QUOTE 2' OR EXISTS (SELECT 1 FROM quote_followup_dispositions d WHERE d.quote_number = q.quote_number))`
    : `q.matched_at IS NULL`;
  const r = await pool.query(
    `UPDATE quote_followup_quotes q SET
       conv_invoice = COALESCE(m.conv_invoice, ''),
       conv_finish = COALESCE(m.conv_finish, ''),
       conv_revenue = m.conv_revenue,
       other_sp = COALESCE(m.other_sp, ''),
       other_invoice = COALESCE(m.other_invoice, ''),
       other_finish = COALESCE(m.other_finish, ''),
       matched_at = NOW()
     FROM (
       SELECT q.quote_number,
              COALESCE(ce.invoice, cl.invoice) AS conv_invoice,
              COALESCE(ce.finish_date, cl.finish_date) AS conv_finish,
              COALESCE(ce.revenue, cl.revenue) AS conv_revenue,
              oth.salesperson_code AS other_sp, oth.invoice AS other_invoice, oth.finish_date AS other_finish
       FROM quote_followup_quotes q
       LEFT JOIN LATERAL (
         SELECT o.invoice, o.finish_date, o.revenue FROM sales_order_detail o
         WHERE q.source <> 'service_lead' AND q.customer_number <> ''
           AND o.customer_number = q.customer_number AND o.salesperson_code = q.sp_code AND o.finish_date >= q.date_created
         ORDER BY o.finish_date, o.invoice LIMIT 1
       ) ce ON TRUE
       LEFT JOIN LATERAL (
         SELECT o.invoice, o.finish_date, o.revenue FROM sales_order_detail o
         WHERE q.source = 'service_lead' AND q.customer_name <> ''
       ...
       LEFT JOIN LATERAL (
         SELECT o.invoice, o.finish_date, o.salesperson_code FROM sales_order_detail o
         WHERE q.source <> 'service_lead' AND q.customer_number <> ''
           AND o.customer_number = q.customer_number AND o.salesperson_code <> q.sp_code AND o.salesperson_code <> ''
           AND o.finish_date >= q.date_created
         ORDER BY o.finish_date, o.invoice LIMIT 1
       ) oth ON TRUE
       WHERE ${where}
     ) m
     WHERE m.quote_number = q.quote_number`,
    list ? [list] : []
  );
...
export async function upsertServiceLeadQuote({ token, svNumber, customerName, phone, applianceText = "", estimateTotal = 0, dateCreated = "" }) {
  ...
  await refreshQuoteConversions([quoteNumber]).catch(() => {});
  return quoteNumber;
}
...
// Split: give these quotes their own project (a fresh key), labelled.
// Unsplit: drop the key so they fall back under the customer rule.
export async function splitQuotesToProject({ quoteNumbers, label = "", byEmail = "", byName = "" }) {
  const pool = await getReadyPool();
  const numbers = (Array.isArray(quoteNumbers) ? quoteNumbers : []).map((n) => text(n).toUpperCase()).filter(Boolean);
  if (!numbers.length) throw new Error("Pick at least one quote to move to its own card.");
  const key = `p_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}`;
  const lbl = text(label).slice(0, 80);
  for (const n of numbers) {
    await pool.query(
      `INSERT INTO quote_followup_projects (quote_number, project_key, label, by_email, by_name, updated_at)
       VALUES ($1, $2, $3, $4, $5, NOW())
       ON CONFLICT (quote_number) DO UPDATE SET project_key = EXCLUDED.project_key, label = EXCLUDED.label, by_email = EXCLUDED.by_email, by_name = EXCLUDED.by_name, updated_at = NOW()`,
      [n, key, lbl, String(byEmail || "").toLowerCase().slice(0, 200), String(byName || "").slice(0, 200)]
    );
  }
  return { projectKey: key, label: lbl, moved: numbers.length };
}
export async function unsplitQuotes(quoteNumbers) {
  const pool = await getReadyPool();
  const numbers = (Array.isArray(quoteNumbers) ? quoteNumbers : []).map((n) => text(n).toUpperCase()).filter(Boolean);
  if (!numbers.length) return { cleared: 0 };
  const r = await pool.query(`DELETE FROM quote_followup_projects WHERE quote_number = ANY($1)`, [numbers]);
...
function groupQuotes(quotes) {
  ...
  for (const q of quotes) {
    // A split quote groups only with its project; the rest by customer.
    const key = q.projectKey ? `${q.spCode}|proj:${q.projectKey}` : `${q.spCode}|${q.customerNumber || q.quoteNumber}`;
  ...
  for (const [, list] of byCustomer) {
    ...
    const isProject = !!list[0].projectKey;
    let current = null;
    for (const q of list) {
      // Projects ignore the 30-day window: one card however long it runs.
      if (!current || (!isProject && q.dateCreated && q.dateCreated > addDays(current.firstDate, 30))) {
  ...
  return groups.map((g) => {
    ...
    return {
      key: `${first.spCode}|${first.customerNumber}|${first.projectKey || g.firstDate}`,
      projectKey: first.projectKey || "",
      projectLabel: g.quotes.map((q) => q.projectLabel).find(Boolean) || "",
      projectBy: g.quotes.map((q) => q.projectBy).find(Boolean) || "",
      spCode: first.spCode,
...
export async function getQuoteFollowupBoard({ spCode = null, from = "", to = "" } = {}) {
  ...
  // First load after deploy (or a quote that slipped in unmatched): compute
  // its cache row now. No-op when everything is matched.
  await refreshQuoteConversions("unmatched").catch(() => {});
  ...
  const result = await pool.query(
    `SELECT q.*,
            ...
            p.project_key, p.label AS project_label, p.by_name AS project_by
     FROM quote_followup_quotes q
     LEFT JOIN quote_followup_dispositions d ON d.quote_number = q.quote_number
     LEFT JOIN quote_followup_projects p ON p.quote_number = q.quote_number
