...
export async function listReturnLines({ startMonth, endMonth }) {
  ...
  const result = await pool.query(
    `SELECT l.*,
     ...
     LEFT JOIN salesperson_codes sc ON sc.code = o.salesperson_code
     LEFT JOIN LATERAL (
       SELECT s.invoice, s.source_month, s.revenue, s.serial_cost, s.salesperson_code,
              CASE
       ...
       WHERE s.qty > 0
         ...
         AND s.id <> l.id
         AND (s.base_invoice = l.base_invoice
  ...
  const MATCH_LABELS = { 1: "invoice + product", 2: "customer + product", 3: "serial cost fingerprint" };
  return result.rows.map((row) => ({
    ...mapLineRow(row),
