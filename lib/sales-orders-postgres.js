...
ALTER TABLE pushed_notifications ADD COLUMN IF NOT EXISTS action_key TEXT NOT NULL DEFAULT '';
`;

// Optional "please do this" attached to a pushed notification. The dashboard
// renders a matching button; completing the action closes the notification
// for that person automatically (closeActionNotificationsForUser).
export const NOTIFICATION_ACTIONS = {
  sizes: { label: "Update my uniform sizes", description: "Asks them to confirm or update the shirt and shoe size on their directory profile. Saving their sizes closes the notification for them." }
};
...
export async function createPushedNotification({ severity, typeLabel, title, body, audienceEmail = "", refId = "", claimable = false, actionKey = "", byEmail, byName }) {
  const pool = await getReadyPool();
  const level = NOTIFICATION_SEVERITIES.includes(severity) ? severity : "neutral";
  const result = await pool.query(
    `INSERT INTO pushed_notifications (severity, type_label, title, body, audience_email, ref_id, claimable, created_by_email, created_by_name, action_key)
    ...
    [
      ...
      String(byEmail || "").trim().toLowerCase(),
      String(byName || "").trim(),
      NOTIFICATION_ACTIONS[actionKey] ? actionKey : ""
    ]
  );
  return mapNotificationRow(result.rows[0]);
}

// Manual sends from the Send a Notification page share a ref ("push:<uuid>"),
// one row per recipient (or one row with a blank audience for everyone).
// This lists them newest first with per-person read receipts: for a
// targeted send, each recipient row and whether they've closed it; for an
// all-hands send, everyone who has closed it so far.
export async function listPushedCampaigns({ limit = 100 } = {}) {
  const pool = await getReadyPool();
  const result = await pool.query(
    `SELECT p.*, c.closed_at AS closed_at, c.closed_by_name AS closed_by_name, c.closed_by_email AS closed_by_email
       FROM pushed_notifications p
       LEFT JOIN sales_order_flag_closures c ON c.kind = 'notification' AND c.ref_id = p.id::text
      WHERE p.ref_id LIKE 'push:%'
      ORDER BY p.created_at DESC, p.audience_email ASC, c.closed_at ASC`
  );
  const groups = new Map();
  for (const row of result.rows) {
    let g = groups.get(row.ref_id);
    if (!g) {
      g = { ...mapNotificationRow(row), id: undefined, audience: row.audience_email ? "users" : "all", recipients: [], closers: [], activeCount: 0, rowCount: 0 };
      delete g.id; delete g.audienceEmail; delete g.claimable; delete g.claimedByEmail; delete g.claimedByName; delete g.claimedAt;
      groups.set(row.ref_id, g);
      if (groups.size > limit) { groups.delete(row.ref_id); break; }
    }
    if (row.audience_email) {
      let r = g.recipients.find((x) => x.email === row.audience_email);
      if (!r) { r = { email: row.audience_email, active: row.active, closedAt: null }; g.recipients.push(r); g.rowCount += 1; if (row.active) g.activeCount += 1; }
      if (row.closed_at && !r.closedAt) r.closedAt = row.closed_at.toISOString?.() || row.closed_at;
    } else {
      if (!g.rowCount) { g.rowCount = 1; g.activeCount = row.active ? 1 : 0; }
      if (row.closed_by_email) g.closers.push({ email: row.closed_by_email, name: row.closed_by_name || "", closedAt: row.closed_at?.toISOString?.() || row.closed_at });
    }
    g.active = g.activeCount > 0;
  }
  return [...groups.values()];
}

// Completing a requested action (e.g. saving uniform sizes) closes every
// active notification carrying that action for the person — the read
// receipt lands in the closure log like a manual Close would.
export async function closeActionNotificationsForUser(actionKey, email, byName = "") {
  ...
  const result = await pool.query(
    `SELECT p.* FROM pushed_notifications p
      WHERE p.active AND p.action_key = $1 AND (p.audience_email = '' OR p.audience_email = $2)
        AND NOT EXISTS (SELECT 1 FROM sales_order_flag_closures c WHERE c.kind = 'notification' AND c.ref_id = p.id::text AND c.closed_by_email = $2)`,
    [key, who]
  );
  for (const row of result.rows) {
    await recordFlagClosure({
      severity: row.severity, invoice: "", signature: `notification:${row.id}`,
      flags: [{ level: row.severity, text: row.type_label }], orderSnapshot: {},
      byEmail: who, byName, kind: "notification", refId: String(row.id), title: row.title
    });
  }
...
// DIBS lifecycle report: one row per claimable lead (the per-consultant
// fan-out copies collapse on ref_id). "Closed" means the CLAIMER pressed
// Close on their copy — that's the loop-closed signal; other users closing
// an unclaimed copy merely hides it for them and is counted separately.
export async function listClaimableLeadReport({ fromDate, toDate }) {
  const pool = await getReadyPool();
  const leads = await pool.query(
    `SELECT ref_id,
            MIN(created_at) AS created_at,
            (ARRAY_AGG(type_label ORDER BY created_at))[1] AS type_label,
            (ARRAY_AGG(title ORDER BY created_at))[1] AS title,
            (ARRAY_AGG(created_by_name ORDER BY created_at))[1] AS created_by_name,
            COUNT(*)::int AS fanned_to,
            MAX(claimed_by_email) AS claimed_by_email,
            MAX(claimed_by_name) AS claimed_by_name,
            MAX(claimed_at) AS claimed_at,
            BOOL_OR(active) AS any_active,
            ARRAY_AGG(id::text) AS row_ids
       FROM pushed_notifications
      WHERE claimable AND ref_id <> ''
        AND created_at >= ($1::date)::timestamp AT TIME ZONE 'America/Chicago'
        AND created_at < (($2::date + INTERVAL '1 day'))::timestamp AT TIME ZONE 'America/Chicago'
      GROUP BY ref_id
      ORDER BY MIN(created_at) DESC
      LIMIT 1000`,
    [String(fromDate || ""), String(toDate || "")]
  );
  const allRowIds = leads.rows.flatMap((r) => r.row_ids);
  const closures = allRowIds.length
    ? await pool.query(
        `SELECT ref_id, closed_by_email, closed_by_name, MIN(closed_at) AS closed_at
           FROM sales_order_flag_closures
          WHERE kind = 'notification' AND ref_id = ANY($1)
          GROUP BY ref_id, closed_by_email, closed_by_name`,
        [allRowIds]
      )
    : { rows: [] };
  const closuresByRow = new Map();
  for (const c of closures.rows) {
    const list = closuresByRow.get(c.ref_id) || [];
    list.push(c);
    closuresByRow.set(c.ref_id, list);
  }
  return leads.rows.map((r) => {
    const rowClosures = r.row_ids.flatMap((id) => closuresByRow.get(id) || []);
    const claimerClosure = r.claimed_by_email
      ? rowClosures.find((c) => c.closed_by_email === r.claimed_by_email)
      : null;
    return {
      refId: r.ref_id,
      ...
      fannedTo: r.fanned_to,
      claimedByEmail: r.claimed_by_email || "",
      claimedByName: r.claimed_by_name || "",
      claimedAt: r.claimed_at?.toISOString?.() || "",
      closedAt: claimerClosure?.closed_at?.toISOString?.() || "",
      closedByName: claimerClosure?.closed_by_name || "",
      dismissedCopies: rowClosures.length,
      active: r.any_active === true
    };
  });
}
...
// The live card for a ref (the living summary flags re-issue themselves on
// every refresh; callers use this to leave an unchanged one alone).
export async function getActivePushedNotificationByRef(refId) {
  const ref = String(refId || "").trim();
  if (!ref) return null;
  const pool = await getReadyPool();
  const result = await pool.query(`SELECT * FROM pushed_notifications WHERE ref_id = $1 AND active ORDER BY id DESC LIMIT 1`, [ref]);
...
// Before the feed overwrites a snapshot that came from a spreadsheet upload,
// keep that upload at id 2 so /api/epass/feed-vs-upload can compare the two.
export async function keepLastSalesUploadCopy() {
  const pool = await getReadyPool();
  const r = await pool.query(`INSERT INTO sales_order_snapshots (id, filename, uploaded_by_email, uploaded_by_name, uploaded_at, row_count, data)
     SELECT 2, filename, uploaded_by_email, uploaded_by_name, uploaded_at, row_count, data FROM sales_order_snapshots WHERE id = 1 AND filename NOT LIKE 'ePASS feed%'
     ON CONFLICT (id) DO UPDATE SET filename = EXCLUDED.filename, uploaded_by_email = EXCLUDED.uploaded_by_email, uploaded_by_name = EXCLUDED.uploaded_by_name, uploaded_at = EXCLUDED.uploaded_at, row_count = EXCLUDED.row_count, data = EXCLUDED.data`);
  return r.rowCount;
}
export async function getLastSalesUploadSnapshot() {
  const pool = await getReadyPool();
  const result = await pool.query(`SELECT * FROM sales_order_snapshots WHERE id = 2`);
  return result.rows[0] ? mapRow(result.rows[0]) : null;
}
