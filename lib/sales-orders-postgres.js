...
export async function recordFlagClosure({ severity, invoice, signature, flags, orderSnapshot, byEmail, byName, reportUploadedAt = null, reportFilename = "" }) {
  const pool = await getReadyPool();
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    ...
    const result = await pool.query(
      `INSERT INTO sales_order_flag_closures
         (token, severity, invoice, signature, flags, order_snapshot, closed_by_email, closed_by_name, report_uploaded_at, report_filename)
...
// YYYY-MM-DD in Central time; end is inclusive.
export async function listFlagClosuresRange({ start, end }) {
  const pool = await getReadyPool();
  const result = await pool.query(
    `SELECT * FROM sales_order_flag_closures
     WHERE closed_at >= ($1::date)::timestamptz AT TIME ZONE 'America/Chicago' AT TIME ZONE 'UTC'
       AND closed_at < (($2::date + INTERVAL '1 day'))::timestamptz AT TIME ZONE 'America/Chicago' AT TIME ZONE 'UTC'
