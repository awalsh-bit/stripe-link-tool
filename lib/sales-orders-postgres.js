...
export async function createPushedNotification({ severity, typeLabel, title, body, audienceEmail = "", refId = "", byEmail, byName }) {
  const pool = await getReadyPool();
  const level = NOTIFICATION_SEVERITIES.includes(severity) ? severity : "neutral";
  const result = await pool.query(
    `INSERT INTO pushed_notifications (severity, type_label, title, body, audience_email, ref_id, created_by_email, created_by_name)
