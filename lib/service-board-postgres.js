...
function mapBoardJob(j, S) {
  ...
  return {
    ...
    owner: j.owner_tech || null, eta: dateStr(j.parts_eta), lock: !!j.locked, forced: !!j.forced, note: j.dispatch_note || "", confirmed: !!j.confirmed_at,
...
export async function getServiceBoard({ from = "", days = 5 } = {}) {
  ...
  return {
    ...
    jobs, blocks, td: TD, confirmed: CONFIRMED, notify: { routeConfirm: S["notify.route_confirm.enabled"] === true, reschedule: S["notify.reschedule.enabled"] === true },
    settings: S,
