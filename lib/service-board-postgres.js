...
function mapBoardJob(j, S) {
  ...
  return {
    ...
    contactPref: j.contact_pref || "", partsIn: j.parts_in_channel ? { channel: String(j.parts_in_channel).replace(/:pending$/, ""), sent: !/:pending$/.test(String(j.parts_in_channel)), at: j.parts_in_notified_at ? new Date(j.parts_in_notified_at).toISOString() : null } : null,
...
export async function getServiceHistory(sv) {
  ...
  // The full ePASS catalogue (every finished ticket, twenty years) when it has
  // been loaded — epass_service_catalogue, fed by the epass-service-catalogue
  // bundle; the 3-year history dataset above stays as the fallback.
  const cat = hClauses.length ? (await pool.query(`SELECT * FROM epass_service_catalogue WHERE ${hClauses.join(" OR ")} ORDER BY date_finished DESC NULLS LAST LIMIT 80`, hParams).catch(() => ({ rows: [] }))).rows : [];
  for (const r of cat) {
    if (seen.has(r.code)) continue; seen.add(r.code);
    const fix = (t) => String(t || "").replace(/([A-Za-z])\?([A-Za-z])/g, "$1'$2");
    out.push({ sv: r.code, source: "epass", st: r.job_status, open: false, created: dateStr(r.date_created), finished: dateStr(r.date_finished), day: "", tech: r.tech_code || "", unit: [r.brand, r.model].filter(Boolean).join(" "), serial: r.serial || "", problem: fix(r.complaint).slice(0, 400), performed: fix(r.performed).slice(0, 400), sameUnit: !!serial && String(r.serial || "").toUpperCase() === serial, total: Number(r.total) || 0, wty: !!r.is_warranty, catalogue: true });
  }
  out.sort((a, b) => String(b.created || "").localeCompare(String(a.created || "")));
  const historyFeed = (await pool.query(`SELECT (SELECT COUNT(*) FROM epass_service_history) + (SELECT COUNT(*) FROM epass_service_catalogue) AS n`).catch(() => ({ rows: [{ n: 0 }] }))).rows[0].n;
  return {
    ...
    visits: out, historyFeedLoaded: Number(historyFeed) > 0
  };
}
...
// Master search (Andrew, 9/22 late): every open call wherever it is — on a
// day, unscheduled, past dated, in the shop, in the lane — plus the last
// week's closed ones so a "where did it go" question has an answer.
export async function searchBoardJobs(q, { limit = 40 } = {}) {
  const pool = await getReadyPool();
  ...
  const rows = (await pool.query(
    `SELECT j.* FROM sj_jobs j
      WHERE NOT (j.in_feed = FALSE AND j.source = 'import' AND j.closed_at < NOW() - INTERVAL '7 days')
        AND (j.closed_at IS NULL OR j.closed_at >= NOW() - INTERVAL '7 days')
        AND (j.sv_number ILIKE $1 OR j.customer_name ILIKE $1 OR j.phone ILIKE $1 OR j.phone_alt ILIKE $1 OR j.address1 ILIKE $1 OR j.zip ILIKE $1
             OR j.unit_model ILIKE $1 OR j.unit_serial ILIKE $1 OR j.unit_brand ILIKE $1 OR j.problem_text ILIKE $1 OR j.parts_po ILIKE $1 OR j.email ILIKE $1)
      ORDER BY (j.closed_at IS NOT NULL), j.route_date NULLS LAST, j.sv_number LIMIT $2`, [needle, limit])).rows;
  return rows.map((j) => ({ ...mapBoardJob(j, S), closed: !!j.closed_at, phone: j.phone || "" }));
}
