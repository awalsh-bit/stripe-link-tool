...
export async function buildServiceCommissionBoard({ year, quarter, today = new Date().toISOString().slice(0, 10), techCode = "" } = {}) {
  ...
  const techs = plans.filter((p) => p.active && (!wantCode || p.techCode === wantCode)).map((plan0) => {
    // this quarter's weekly quota (set by quarter; the year number is the fallback)
    const plan = { ...plan0, weeklyQuota: weeklyQuotaFor(plan0, q.q), yearWeeklyQuota: plan0.weeklyQuota };
