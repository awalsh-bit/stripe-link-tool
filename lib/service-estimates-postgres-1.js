...
function parseWorkOrderRows(rows) {
  ...
  for (const r of rows) {
    ...
    if (/\b(payments?|deposits?)\b/i.test(joinedAll) && !/adjustment/i.test(joinedAll)) { out.deposits = Math.round(((out.deposits || 0) + ext) * 100) / 100; continue; }
...
export function assessPartsQuality(summary) {
  ...
  const labor = Number(summary?.laborEntries || 0) > 0;
  const problems = [];
  parts.forEach((p, i) => {
...
export function extractServiceEstimateFromPdf(buffer) {
  ...
  for (const r of flat) {
    ...
    else if (/\b(DEPOSITS?|PAYMENTS?)\b/.test(labelText) && !/ADJUSTMENT/.test(labelText)) out.deposits = Math.round(((out.deposits || 0) + amount) * 100) / 100;
