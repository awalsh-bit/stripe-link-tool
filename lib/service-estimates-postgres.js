...
//
// BACKLOG (Andrew, 2026-09-17 — noted, not built): an "installed outside
//   manufacturer recommendation" flag alongside "repair not recommended" —
//   the tech marks that the unit is installed outside the manufacturer's
//   guidance (location / clearances / venting / power), the client sees the
//   note on the estimate, and the response records that they were told.
//   Confirm with Andrew whether it also gates the warranty language.
// ---------------------------------------------------------------------------
...
ALTER TABLE service_estimates ADD COLUMN IF NOT EXISTS sends JSONB NOT NULL DEFAULT '[]'::jsonb;
...
//   core         "CORE CHARGE  CORE CHARGE - RETURN" — the deposit on a part
//                with an exchangeable core (SV00123219, 9/22). Not a part: it
//                usually appears twice, +1 and −1, netting to zero. The net
//                goes to coreTotal; the client sees one "Core charge" line only
...
export function classifyEstimateLine(joined) {
  ...
  if (/^core\s*charge\b/i.test(t)) return "core";
  return "part";
}
function pushCore(out, { qty, ext }) {
  out.coreCharges = out.coreCharges || [];
  out.coreCharges.push({ qty, ext });
  out.coreTotal = Math.round(((out.coreTotal || 0) + (ext || 0)) * 100) / 100;
}
...
function parseWorkOrderRows(rows) {
  ...
  if (!out.noLinePricing && out.subTotal != null) {
    const sum = Math.round((partsTotal + out.laborTotal + out.shippingTotal + (out.refrigerantTotal || 0) + (out.coreTotal || 0)) * 100) / 100;
...
// Store
// ---------------------------------------------------------------------------
// Two statuses, kept apart on purpose (Andrew, 2026-09-17):
//   COMMUNICATION — what WE have done: unsent → sent → reminder sent (×N).
//     Derived from the `sends` log (every explicit email/text click; older
//     rows that only carry emailed_at count as one email send).
//   CLIENT — what THEY have done: unseen → viewed → approved / shopping /
//     went elsewhere (or a staff resolution: diagnostic / comped).
//     Derived from `status`, which stays the workflow field it always was.
function normalizeSends(row) {
  const raw = Array.isArray(row.sends) ? row.sends : [];
  const sends = raw.map((x) => ({ at: String(x.at || ""), channel: x.channel === "text" ? "text" : "email", variant: x.variant === "reminder" ? "reminder" : "standard", to: String(x.to || ""), by: String(x.by || "") })).filter((x) => x.at);
  const emailedAt = row.emailed_at?.toISOString?.() || null;
  if (!sends.length && emailedAt) sends.push({ at: emailedAt, channel: "email", variant: "standard", to: row.contact_email || "", by: "" });
  return sends.sort((a, b) => a.at.localeCompare(b.at));
}
export function communicationFor(sends) {
  const reminders = sends.filter((x) => x.variant === "reminder").length;
  const state = !sends.length ? "unsent" : reminders ? "reminded" : "sent";
  const label = state === "unsent" ? "Unsent" : state === "sent" ? "Sent" : `Reminder sent ×${reminders}`;
  return { state, label, sentAt: sends[0]?.at || null, lastSentAt: sends[sends.length - 1]?.at || null, lastChannel: sends[sends.length - 1]?.channel || "", sends: sends.length, reminders };
}
const CLIENT_LABELS = { unseen: "Unseen", viewed: "Viewed", approved: "Approved", shopping: "Shopping", elsewhere: "Went elsewhere", diagnostic: "Diagnostic only", comped: "Comped" };
export function clientStatusFor(row) {
  const status = String(row.status || "sent");
  const state = status === "sent" ? "unseen" : status;
  return { state, label: CLIENT_LABELS[state] || state, viewedAt: row.viewed_at?.toISOString?.() || null, respondedAt: row.responded_at?.toISOString?.() || null, byStaff: !!row.response?.resolvedByStaff };
}

function mapEstimate(row) {
  const sends = normalizeSends(row);
  return {
    sends,
    communication: communicationFor(sends),
    client: clientStatusFor(row),
    id: Number(row.id),
    token: row.token,
...
// Every explicit send (email or text, first send or reminder) lands in the
// `sends` log; an email also keeps emailed_at / contact_email current for
// the older code paths that read them.
export async function recordServiceEstimateSend(token, { channel = "email", variant = "standard", to = "", by = "" } = {}) {
  const pool = await getReadyPool();
  const entry = { at: new Date().toISOString(), channel: channel === "text" ? "text" : "email", variant: variant === "reminder" ? "reminder" : "standard", to: String(to || "").slice(0, 200), by: String(by || "").slice(0, 200) };
  const r = await pool.query(
    `UPDATE service_estimates
     SET sends = COALESCE(sends, '[]'::jsonb) || $2::jsonb,
         emailed_at = CASE WHEN $3 = 'email' THEN COALESCE(emailed_at, NOW()) ELSE emailed_at END,
         contact_email = CASE WHEN $3 = 'email' THEN COALESCE(NULLIF($4, ''), contact_email) ELSE contact_email END
     WHERE token = $1 RETURNING *`,
    [String(token || ""), JSON.stringify([entry]), entry.channel, entry.channel === "email" ? String(to || "").trim().toLowerCase().slice(0, 200) : ""]
  );
  return r.rows[0] ? mapEstimate(r.rows[0]) : null;
}
export async function markServiceEstimateEmailed(token, contactEmail) {
  return recordServiceEstimateSend(token, { channel: "email", variant: "standard", to: contactEmail });
}
