...
function parseWorkOrderRows(rows) {
  ...
  for (const r of rows) {
    for (const c of r.cells) {
      const t = c.t.trim();
      if (!out.svNumber && /^SV\d{6,}$/.test(t)) out.svNumber = t;
    }
  }
  ...
  // Bill To name: first LASTNAME, FIRST style cell below the Bill To label.
  const billToRow = rows.find((r) => r.cells.some((c) => /^Bill To:$/.test(c.t.trim())));
  if (billToRow) {
    ...
    for (const r of below) {
      ...
      if (nameCell) { out.customerName = properCase(nameCell.t.trim()); break; }
    }
  }
  const svcAtRow = rows.find((r) => r.cells.some((c) => /^Service At:$/.test(c.t.trim())));
  if (svcAtRow) {
    const num = svcAtRow.cells.find((c) => /^\d{10}$/.test(c.t.trim()));
    if (num) out.customerNumber = num.t.trim();
  }
  ...
  const partsTotal = Math.round(out.parts.reduce((sum, p) => sum + p.ext, 0) * 100) / 100;
...
export function extractServiceEstimateFromPdf(buffer) {
  ...
  if (billTo) out.customerName = properCase(billTo.cells.find((c) => c.x > 1000 && c.x < 2200)?.t.trim() || "");
  for (const r of flat) {
...
export async function createServiceEstimate({ svNumber, estimateName, customerName, customerNumber, contactPhone, contactEmail, contactPref = "", summary, byEmail, byName }) {
  ...
  const result = await pool.query(
    ...
    [
      ...
      ["call", "text"].includes(contactPref) ? contactPref : "",
