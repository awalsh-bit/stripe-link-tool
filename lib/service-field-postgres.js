...
import { fiscalPeriodFor, listServiceCompPlans, getServiceCompSettings, svTicketsBetween } from "./service-commissions-postgres.js";
...
export async function serialTagStatus(job) {
  ...
  if (s.length >= 4) {
    if (await serialSeenBefore(s).catch(() => false)) return { onFile: true, why: "we have serviced this serial before (ePASS history)", photoId: null };
    const prior = (await p.query(`SELECT sv_number FROM sj_jobs WHERE unit_serial = $1 AND sv_number <> $2 AND closed_at IS NOT NULL LIMIT 1`, [job.unit_serial, job.sv_number])).rows[0];
    if (prior) return { onFile: true, why: `serviced before (${prior.sv_number})`, photoId: null };
  }
  return { onFile: false, why: s ? "no serial tag on file" : "no serial on the ticket — photograph the tag", photoId: null };
}
...
export async function dayDollars({ tech, date }) {
  ...
  try {
    ...
    if (w) {
      ...
      if (plan?.weeklyQuota) { quota = plan.weeklyQuota; quotaSource = "weekly quota"; }
      else {
...
export async function verifyParts({ sv, lines = [], labor = null, note = "", by = "", byName = "" }) {
  ...
  return { ok: true, status: "SO2.1", lines: clean, labor: cleanL, partsTotal: r2(total), laborTotal: r2(laborTotal), syncId: r.syncId };
}
