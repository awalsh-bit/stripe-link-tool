...
async function applyStatusRules(client, sv, { from = null, settings = {}, created = false } = {}) {
  ...
  // Parts in (or install scheduled with no date): the owning tech's first
  // open day, never anyone else's — placed as a pencil, the dispatcher
  // confirms the route as usual.
  if (to === "SO5" || (to === "SO6" && !job.route_date)) {
    ...
    if (to === "SO5" && (from !== "SO5")) { changes.route_date = null; changes.time_window = ""; changes.stop_seq = null; changes.confirmed_at = null; }
    if (owner) changes.assigned_tech = owner;
    await updateJob(client, sv, changes);
    ...
    const eta = fresh.parts_eta ? dateStr(fresh.parts_eta) : null;
    const pick = await autoPlace(client, fresh, { techOnly: owner, earliest: eta, why: "Parts in — install with the tech who diagnosed it" });
