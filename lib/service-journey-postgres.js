...
async function upsertFromEpassFeed(client, sv, inv, labor, items, batchId) {
  ...
  // The tech: the latest trip's labor line, else the route the ticket was requested on.
  const lastLabor = [...labor].reverse().find((l) => l.tech_code);
  const truck = await canonicalTech(client, lastLabor?.tech_code || rawStr(inv, "DispatchRequestedRouteCode"));
