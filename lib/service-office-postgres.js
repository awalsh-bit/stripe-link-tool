...
export async function listPartsQueue() {
  ...
  const rows = (await pool.query(
    `SELECT j.*, t.name AS tech_name, o.name AS owner_name,
            ...
            (SELECT COALESCE(json_agg(json_build_object('item', i.item_code, 'supplier', NULLIF(COALESCE(i.raw->>'OrderFromSupplierCode', i.raw->>'SupplierCode'), ''), 'poDated', NULLIF(LEFT(i.raw->>'PODateStamp', 10), ''), 'status', i.raw->>'Status', 'qty', i.raw->>'QtyOrdered') ORDER BY i.id), '[]'::json) FROM epass_open_service_items i WHERE i.invoice_code = j.sv_number AND (i.raw->>'PODateStamp' IS NOT NULL OR i.raw->>'SupplierCode' IS NOT NULL)) AS item_pos
     FROM sj_jobs j
  ...
  const poFeed = (await pool.query(`SELECT COUNT(*)::int AS n, COUNT(DISTINCT invoice_code)::int AS tickets FROM epass_open_service_po`)).rows[0];
  const poPaths = (await pool.query(`SELECT COALESCE(raw->>'link_path','backorder') AS path, COUNT(*)::int AS n FROM epass_open_service_po GROUP BY 1 ORDER BY 1`).catch(() => ({ rows: [] }))).rows;
  const pending = (await pool.query(`SELECT COUNT(*)::int AS n, COUNT(DISTINCT invoice_code)::int AS tickets,
      COUNT(*) FILTER (WHERE NULLIF(raw->>'PODateStamp','') IS NOT NULL)::int AS with_stamp,
      COUNT(*) FILTER (WHERE EXISTS (SELECT 1 FROM epass_open_service_po po WHERE po.invoice_code = pp.invoice_code AND po.item_code = pp.item_code))::int AS linked
    FROM epass_open_service_parts_pending pp`).catch(() => ({ rows: [{ n: null }] }))).rows[0];
  const meta = (await pool.query(`SELECT pulled_at, received_at FROM epass_open_service_meta WHERE id = 1`).catch(() => ({ rows: [] }))).rows[0];
  const out = rows.map((j) => {
    ...
    return {
      ...
      // Fallback when no PO line matched: the part lines on the ticket itself
      // carry the supplier code and the date the PO was raised.
      itemPos: Array.isArray(j.item_pos) ? j.item_pos.filter((x) => x.supplier || x.poDated) : [],
  ...
  out.poFeed = { lines: Number(poFeed?.n) || 0, tickets: Number(poFeed?.tickets) || 0, pulledAt: meta?.pulled_at || null, receivedAt: meta?.received_at ? new Date(meta.received_at).toISOString() : null,
    paths: Object.fromEntries(poPaths.map((r) => [r.path, r.n])),
    pending: pending?.n == null ? null : { lines: Number(pending.n) || 0, tickets: Number(pending.tickets) || 0, withStamp: Number(pending.with_stamp) || 0, linked: Number(pending.linked) || 0 } };
  return out;
}
