...
export async function keepLastServiceUploadCopy() {
  const pool = await getReadyPool();
  const r = await pool.query(`INSERT INTO service_order_snapshots (id, filename, uploaded_by_email, uploaded_by_name, uploaded_at, row_count, data)
     SELECT 2, filename, uploaded_by_email, uploaded_by_name, uploaded_at, row_count, data FROM service_order_snapshots WHERE id = 1 AND filename NOT LIKE 'ePASS feed%'
...
export async function getLastServiceUploadSnapshot() {
  const pool = await getReadyPool();
  const result = await pool.query(`SELECT * FROM service_order_snapshots WHERE id = 2`);
  return result.rows[0] ? mapRow(result.rows[0]) : null;
}
