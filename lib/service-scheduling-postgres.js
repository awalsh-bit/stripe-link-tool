...
async function dayStops(pool, S, techCode, date, exceptHoldId = null, cache = new Map()) {
  ...
  for (const j of jobs) {
    ...
    stops.push({ ref: j.sv_number, zone: j.zone_code, point: j.lat != null && j.lng != null ? [Number(j.lat), Number(j.lng)] : null, zip: j.zip, minutes: j.est_minutes != null ? Number(j.est_minutes) : estMinutes(S, { skill, units: Number(j.units) || 1, install }) });
  }
...
export async function suggestForRequest({ zip, skill = "appliance", units = 1, today = todayCentral(), earliest = null, exceptHoldId = null, limit = 500 }) {
  ...
  for (let i = 0; i < S["placement.horizon_business_days"]; i++) {
    for (const t of techs) {
      ...
      if (t.max_stops && stops.length >= Number(t.max_stops)) continue;
