...
export const PLACEMENT_DEFAULTS = {
  ...
  "booking.self_schedule_enabled": true,
...
async function dayStops(pool, S, techCode, date, exceptHoldId = null, cache = new Map()) {
  const jobs = (await pool.query(
    `SELECT sv_number, zone_code, lat, lng, zip, units, unit_category, status FROM sj_jobs
  ...
  for (const j of jobs) {
    ...
    stops.push({ ref: j.sv_number, zone: j.zone_code, point: j.lat != null && j.lng != null ? [Number(j.lat), Number(j.lng)] : null, zip: j.zip, minutes: estMinutes(S, { skill, units: Number(j.units) || 1, install }) });
  }
...
export async function suggestForRequest({ zip, skill = "appliance", units = 1, today = todayCentral(), earliest = null, exceptHoldId = null, limit = 500 }) {
  const pool = await getReadyPool();
  const S = await loadSettings();
  const z = await zoneForZip(pool, zip);
  const here = await locate(pool, z.zip, z.zone);
  const need = estMinutes(S, { skill, units });
  const techs = (await pool.query(`SELECT * FROM sj_techs WHERE active = TRUE AND auto_schedule = TRUE ORDER BY sp_code`)).rows
  ...
  for (let i = 0; i < S["placement.horizon_business_days"]; i++) {
    for (const t of techs) {
      ...
      const stops = await dayStops(pool, S, t.sp_code, d, exceptHoldId, cache);
      ...
      const cap = S["placement.shift_min"] + (ov?.capacity_adjust_min || 0);
      const remaining = cap - work - dr;
      const [added, at] = marginalDrive(S, pts, here, SHOP, SHOP);
      if (remaining - need - added < S["placement.min_slack_min"]) continue;
      ...
      const why = (same ? `${t.sp_code} has ${same} stop${same === 1 ? "" : "s"} in ${z.zoneCode} that day` : n ? `${t.sp_code} has ${n} stop${n === 1 ? "" : "s"} that day` : `${t.sp_code}'s day is empty`) + ` · +${added} min drive · ${hm(remaining - need - added)} left` + (rank === 0 ? " · primary tech" : rank === 1 ? " · secondary" : " · not this zone's tech");
      out.push({ tech: t.sp_code, date: d, window: at <= Math.floor(Math.max(pts.length, 1) / 2) ? "AM" : "PM", cost: Math.round(cost), addedDrive: added, sameZone: same, stops: n, remainingAfter: remaining - need - added, zoneRank: rank, waitDays: wait, why });
    }
