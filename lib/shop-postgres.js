...
// Staff-side search for the dashboard's shopper admin: name, email, client
// code, or phone digits. Empty search → the 20 newest profiles.
export async function searchShopShoppers(search) {
  const pool = await getReadyPool();
  const raw = String(search || "").trim();
  let result;
  if (!raw) {
    result = await pool.query(`SELECT * FROM shop_shoppers ORDER BY created_at DESC LIMIT 20`);
  } else {
