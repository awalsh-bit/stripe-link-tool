...
-- Days of the week the same-day install program runs (0 = Sunday ... 6 =
-- Saturday). Off days offer no same-day and no fast next-day slots.
ALTER TABLE shop_express_settings ADD COLUMN IF NOT EXISTS days JSONB NOT NULL DEFAULT '[1,2,3,4,5,6]';
`;
...
// Every serial→ticket pairing the snapshots have EVER seen, kept forever.
// The inventory export only shows units still in stock — once a unit is
// delivered/invoiced its row disappears, which is exactly when an installer
// is standing in front of it filing a damage report. This history is what
// the install-damage lookup reads, so "what was on this order" survives the
// unit leaving inventory.
const SERIAL_HISTORY_SCHEMA_SQL = `
CREATE TABLE IF NOT EXISTS shop_serial_written_history (
  invoice_base TEXT NOT NULL,
  ...
  PRIMARY KEY (invoice_base, model, serial)
);
`;

async function recordSerialWrittenHistory(pool, cleanUnits) {
  const rows = [];
  const seen = new Set();
  for (const u of cleanUnits) {
    const base = String(u.writtenTo || "").trim().toUpperCase().replace(/-\d+$/, "");
    if (!base) continue;
    const model = String(u.model || u.sku || "").trim();
    const key = `${base}|${model}|${u.serial}`;
    if (seen.has(key)) continue;
    seen.add(key);
    rows.push({ base, model, serial: u.serial, brand: u.brand || "", description: u.description || "", prod: u.prod || "" });
  }
  if (!rows.length) return 0;
  await pool.query(SERIAL_HISTORY_SCHEMA_SQL);
  await pool.query(
    `INSERT INTO shop_serial_written_history (invoice_base, model, serial, brand, description, prod, last_seen)
     SELECT x.base, x.model, x.serial, x.brand, x.description, x.prod, NOW()
       FROM jsonb_to_recordset($1::jsonb) AS x(base TEXT, model TEXT, serial TEXT, brand TEXT, description TEXT, prod TEXT)
     ON CONFLICT (invoice_base, model, serial) DO UPDATE SET
...
// Model → brand map, accumulated from every inventory snapshot (the serial
// export carries Brand + Description per model; the commission report the
// Brand Sales report is built on does not). Once a model has EVER passed
// through inventory, the report can attribute it.
const MODEL_BRAND_SCHEMA_SQL = `
CREATE TABLE IF NOT EXISTS model_brand_map (
...
-- Where a row came from: 'snapshot' (accumulated from the inventory feed)
-- or 'netsuite' (the item-catalog import — authoritative; snapshot rows
-- never overwrite it).
ALTER TABLE model_brand_map ADD COLUMN IF NOT EXISTS source TEXT NOT NULL DEFAULT 'snapshot';
-- NetSuite merchandising hierarchy (Merch Category / Class / Subclass on the
-- item export, e.g. "15 Kitchen Appliances" / "120 Dishwasher" /
-- "1330 Built-in Dishwasher") - the Express Assortment catalog filters on it.
ALTER TABLE model_brand_map ADD COLUMN IF NOT EXISTS merch_category TEXT NOT NULL DEFAULT '';
ALTER TABLE model_brand_map ADD COLUMN IF NOT EXISTS merch_class TEXT NOT NULL DEFAULT '';
ALTER TABLE model_brand_map ADD COLUMN IF NOT EXISTS merch_subclass TEXT NOT NULL DEFAULT '';
CREATE TABLE IF NOT EXISTS model_brand_catalog_meta (
...
async function recordModelBrands(pool, cleanUnits) {
  const byModel = new Map();
  for (const u of cleanUnits) {
    const model = String(u.model || u.sku || "").trim().toUpperCase();
    const brand = String(u.brand || "").trim();
    if (!model || !brand) continue;
    if (!byModel.has(model)) byModel.set(model, { model, brand, description: u.description || "", prod: u.prod || "" });
  }
  if (!byModel.size) return 0;
  await pool.query(MODEL_BRAND_SCHEMA_SQL);
  await pool.query(
    `INSERT INTO model_brand_map (model, brand, description, prod, last_seen, source)
     SELECT x.model, x.brand, x.description, x.prod, NOW(), 'snapshot'
       FROM jsonb_to_recordset($1::jsonb) AS x(model TEXT, brand TEXT, description TEXT, prod TEXT)
     ON CONFLICT (model) DO UPDATE SET
       brand = EXCLUDED.brand,
       description = EXCLUDED.description,
       prod = EXCLUDED.prod,
       last_seen = NOW()
     WHERE model_brand_map.source <> 'netsuite'`,
    [JSON.stringify([...byModel.values()])]
  );
  return byModel.size;
}

// NetSuite item-catalog import (epass-uploads.html): the authoritative
// model→brand source. Rows: [{ model, brand, description, prod? }].
// Overwrites snapshot-derived rows; a blank prod keeps whatever's stored.
export async function importModelBrandCatalog({ rows, filename = "", byEmail = "" }) {
  const byModel = new Map();
  for (const r of Array.isArray(rows) ? rows : []) {
    const model = String(r?.model || "").trim().toUpperCase().slice(0, 80);
    const brand = String(r?.brand || "").trim().slice(0, 60);
    if (!model || !brand || byModel.has(model)) continue;
    byModel.set(model, {
      model, brand,
      description: String(r?.description || "").trim().slice(0, 160),
      prod: String(r?.prod || "").trim().slice(0, 30),
      merch_category: String(r?.merchCategory || "").trim().slice(0, 80),
      merch_class: String(r?.merchClass || "").trim().slice(0, 80),
      merch_subclass: String(r?.merchSubclass || "").trim().slice(0, 80)
    });
  }
  if (!byModel.size) return { imported: 0, added: 0, updated: 0 };
  const pool = await getReadyPool();
  await pool.query(MODEL_BRAND_SCHEMA_SQL);
  const batch = [...byModel.values()];
  let added = 0;
  for (let i = 0; i < batch.length; i += 2000) {
    const result = await pool.query(
      `INSERT INTO model_brand_map (model, brand, description, prod, merch_category, merch_class, merch_subclass, last_seen, source)
       SELECT x.model, x.brand, x.description, x.prod, x.merch_category, x.merch_class, x.merch_subclass, NOW(), 'netsuite'
         FROM jsonb_to_recordset($1::jsonb) AS x(model TEXT, brand TEXT, description TEXT, prod TEXT, merch_category TEXT, merch_class TEXT, merch_subclass TEXT)
       ON CONFLICT (model) DO UPDATE SET
         brand = EXCLUDED.brand,
         description = CASE WHEN EXCLUDED.description <> '' THEN EXCLUDED.description ELSE model_brand_map.description END,
         prod = CASE WHEN EXCLUDED.prod <> '' THEN EXCLUDED.prod ELSE model_brand_map.prod END,
         merch_category = CASE WHEN EXCLUDED.merch_category <> '' THEN EXCLUDED.merch_category ELSE model_brand_map.merch_category END,
         merch_class = CASE WHEN EXCLUDED.merch_class <> '' THEN EXCLUDED.merch_class ELSE model_brand_map.merch_class END,
         merch_subclass = CASE WHEN EXCLUDED.merch_subclass <> '' THEN EXCLUDED.merch_subclass ELSE model_brand_map.merch_subclass END,
         last_seen = NOW(),
         source = 'netsuite'
       RETURNING (xmax = 0) AS inserted`,
      [JSON.stringify(batch.slice(i, i + 2000))]
    );
    added += result.rows.filter((r) => r.inserted).length;
  }
  await pool.query(
    `INSERT INTO model_brand_catalog_meta (id, filename, uploaded_by, uploaded_at, row_count)
     VALUES (1, $1, $2, NOW(), $3)
     ON CONFLICT (id) DO UPDATE SET filename = $1, uploaded_by = $2, uploaded_at = NOW(), row_count = $3`,
    [String(filename).slice(0, 200), String(byEmail).toLowerCase(), byModel.size]
  );
  return { imported: byModel.size, added, updated: byModel.size - added };
}

// For the Upload Center status card: how big the map is and where it's from.
export async function getModelBrandStats() {
  const pool = await getReadyPool();
  try {
    const [counts, meta] = await Promise.all([
      pool.query(`SELECT source, COUNT(*)::int AS n FROM model_brand_map GROUP BY source`),
      pool.query(`SELECT * FROM model_brand_catalog_meta WHERE id = 1`)
    ]);
    const bySource = {};
    for (const row of counts.rows) bySource[row.source || "snapshot"] = row.n;
    const m = meta.rows[0];
    return {
      total: counts.rows.reduce((s, r) => s + r.n, 0),
      netsuite: bySource.netsuite || 0,
      snapshot: bySource.snapshot || 0,
      lastImport: m ? { filename: m.filename, uploadedBy: m.uploaded_by, uploadedAt: m.uploaded_at?.toISOString?.() || null, rowCount: m.row_count } : null
    };
  } catch {
    return { total: 0, netsuite: 0, snapshot: 0, lastImport: null };
  }
}

// { MODEL: { brand, description, prod, source, merchCategory, merchClass, merchSubclass } } — the whole accumulated map.
export async function getModelBrandMap() {
  const pool = await getReadyPool();
  try {
    let result;
    try { result = await pool.query(`SELECT model, brand, description, prod, source, merch_category, merch_class, merch_subclass FROM model_brand_map`); }
    catch { result = await pool.query(`SELECT model, brand, description, prod, source FROM model_brand_map`); } // columns arrive with the next import
    const map = {};
    for (const row of result.rows) {
      map[row.model] = { brand: row.brand, description: row.description, prod: row.prod, source: row.source || "snapshot", merchCategory: row.merch_category || "", merchClass: row.merch_class || "", merchSubclass: row.merch_subclass || "" };
    }
    return map;
  } catch {
    return {}; // the table appears with the first snapshot save
  }
}

// Units ever written to a sales order (base-invoice keyed), newest first.
export async function listWrittenUnitsForInvoice(invoiceBase) {
  const base = String(invoiceBase || "").trim().toUpperCase().replace(/-\d+$/, "");
  if (!base) return [];
  const pool = await getReadyPool();
  try {
    const result = await pool.query(
      `SELECT * FROM shop_serial_written_history WHERE invoice_base = $1 ORDER BY last_seen DESC, serial ASC LIMIT 12`,
      [base]
    );
    return result.rows.map((r) => ({
      model: r.model || "", serial: r.serial || "", brand: r.brand || "",
      description: r.description || "", prod: r.prod || ""
    }));
  } catch {
    return []; // the table appears with the first snapshot save
  }
...
export async function saveShopInventorySnapshot({ serials, types, written, units, sourceFile, uploadedBy }) {
  ...
  const cleanUnits = (Array.isArray(units) ? units : []).slice(0, 30000).map((u) => ({
    ...
    reserved: String(u?.reserved || "").trim().slice(0, 12),
    orderedFor: String(u?.orderedFor || "").trim().slice(0, 40), // ordered for an invoice that is no longer open (released)
    ...
    listCode: String(u?.listCode || "").trim().slice(0, 30),
    map: Number.isFinite(Number(u?.map)) ? Math.round(Number(u.map) * 100) / 100 : 0,
    mapCode: String(u?.mapCode || "").trim().slice(0, 30),
    l1: Number.isFinite(Number(u?.l1)) ? Math.round(Number(u.l1) * 100) / 100 : 0
  })).filter((u) => u.serial);
  ...
  // Best-effort: the permanent serial→ticket history rides along with every
  // snapshot save; a hiccup here never blocks the availability refresh.
  let historyCount = 0;
  try {
    historyCount = await recordSerialWrittenHistory(pool, cleanUnits);
  } catch (err) {
    console.error("Serial written-history record failed:", err.message);
  }
  let brandCount = 0;
  try {
    brandCount = await recordModelBrands(pool, cleanUnits);
  } catch (err) {
    console.error("Model-brand map record failed:", err.message);
  }
  return { count: clean.length, typedCount: Object.keys(cleanTypes).length, writtenCount: Object.keys(cleanWritten).length, unitDetailCount: cleanUnits.length, historyCount, brandCount };
}
...
const cleanDays = (days) => [...new Set((Array.isArray(days) ? days : []).map(Number).filter((d) => Number.isInteger(d) && d >= 0 && d <= 6))].sort();
...
export async function getShopExpressSettings() {
  ...
  return {
    ...
    days: Array.isArray(row.days) ? cleanDays(row.days) : [...SHOP_EXPRESS_DEFAULTS.days],
...
export async function saveShopExpressSettings({ cutoffHour, products, days, byEmail = "" }) {
  const pool = await getReadyPool();
  const hour = Math.min(23, Math.max(1, Math.round(Number(cutoffHour) || SHOP_EXPRESS_DEFAULTS.cutoffHour)));
  const dayList = days === undefined ? null : cleanDays(days);
  const clean = [...new Set((Array.isArray(products) ? products : [])
    .map((p) => String(p || "").trim().slice(0, 40))
    .filter((p) => /^[\w &-]+\|[\w &-]+$/.test(p)))].slice(0, 100);
  await pool.query(
    `INSERT INTO shop_express_settings (id, cutoff_hour, products, days, updated_by, updated_at)
     VALUES (1, $1, $2::jsonb, COALESCE($4::jsonb, '[1,2,3,4,5,6]'::jsonb), $3, NOW())
     ON CONFLICT (id) DO UPDATE SET
       cutoff_hour = EXCLUDED.cutoff_hour,
       products = EXCLUDED.products,
       days = COALESCE($4::jsonb, shop_express_settings.days),
       updated_by = EXCLUDED.updated_by,
       updated_at = NOW()`,
    [hour, JSON.stringify(clean), String(byEmail || "").trim().toLowerCase().slice(0, 200), dayList == null ? null : JSON.stringify(dayList)]
  );
...
// ---------------------------------------------------------------------------
// Express Assortment — per-MODEL settings for the in-stock listings that come
// from the ePASS feed snapshot (Andrew 9/24: "post everything in stock").
//   price        — sell price override; null = the model's ePASS list price
//   show_single  — list the model even when only ONE unit is in stock (off by
//                  default: one washer is not one built-in refrigerator)
//   hidden       — keep the model off the store regardless of stock
// Keyed on the normalized model (uppercase alphanumerics).
// ---------------------------------------------------------------------------
const MODEL_SETTINGS_SCHEMA_SQL = `
CREATE TABLE IF NOT EXISTS shop_model_settings (
  model TEXT PRIMARY KEY,
  price NUMERIC(12,2),
  show_single BOOLEAN NOT NULL DEFAULT FALSE,
...
`;
let modelSettingsEnsured = null;
async function modelSettingsPool() {
  const pool = await getReadyPool();
  if (!modelSettingsEnsured) modelSettingsEnsured = pool.query(MODEL_SETTINGS_SCHEMA_SQL).catch((e) => { modelSettingsEnsured = null; throw e; });
  await modelSettingsEnsured;
  return pool;
}
const normModelKey = (v) => String(v || "").toUpperCase().replace(/[^0-9A-Z]/g, "");

// { MODELKEY: { price, showSingle, hidden, note, updatedBy, updatedAt } }
export async function listShopModelSettings() {
  const pool = await modelSettingsPool();
  const result = await pool.query(`SELECT * FROM shop_model_settings`);
  const map = {};
  for (const r of result.rows) {
    map[r.model] = { price: r.price == null ? null : Number(r.price), showSingle: !!r.show_single, hidden: !!r.hidden, note: r.note || "", updatedBy: r.updated_by || "", updatedAt: r.updated_at?.toISOString?.() || null };
  }
  return map;
}

export async function saveShopModelSetting({ model, price = null, showSingle = false, hidden = false, note = "", byEmail = "" }) {
  const key = normModelKey(model);
  if (!key) throw new Error("A model is required.");
  const pool = await modelSettingsPool();
  const p = price == null || price === "" ? null : Math.round(Number(price) * 100) / 100;
  if (p != null && (!Number.isFinite(p) || p < 0 || p > 1000000)) throw new Error("Enter a valid price.");
  // Nothing set → drop the row so the model follows the defaults again.
  if (p == null && !showSingle && !hidden && !String(note || "").trim()) {
    await pool.query(`DELETE FROM shop_model_settings WHERE model = $1`, [key]);
    return null;
  }
  const result = await pool.query(
    `INSERT INTO shop_model_settings (model, price, show_single, hidden, note, updated_by, updated_at)
     VALUES ($1, $2, $3, $4, $5, $6, NOW())
     ON CONFLICT (model) DO UPDATE SET price = EXCLUDED.price, show_single = EXCLUDED.show_single, hidden = EXCLUDED.hidden, note = EXCLUDED.note, updated_by = EXCLUDED.updated_by, updated_at = NOW()
     RETURNING *`,
    [key, p, !!showSingle, !!hidden, String(note || "").slice(0, 200), String(byEmail || "").slice(0, 120)]
  );
  const r = result.rows[0];
  return { model: r.model, price: r.price == null ? null : Number(r.price), showSingle: !!r.show_single, hidden: !!r.hidden, note: r.note || "" };
}

// ---------------------------------------------------------------------------
// Delivery ZIPs the online shop serves (Andrew 9/24) — was the
// SHOP_ALLOWED_ZIPS env var; now edited on Shop Orders. Seeded ONCE from that
// list (so emptying the table later isn't undone by a restart).
// ---------------------------------------------------------------------------
const ZIP_SCHEMA_SQL = `
CREATE TABLE IF NOT EXISTS shop_allowed_zips (
  zip TEXT PRIMARY KEY,
  label TEXT NOT NULL DEFAULT '',
  added_by TEXT NOT NULL DEFAULT '',
  added_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
CREATE TABLE IF NOT EXISTS shop_allowed_zips_meta (
  id INT PRIMARY KEY DEFAULT 1 CHECK (id = 1),
  seeded_at TIMESTAMPTZ
);
`;
let zipSchemaDone = null;
async function zipPool() {
  const pool = await getReadyPool();
  if (!zipSchemaDone) zipSchemaDone = pool.query(ZIP_SCHEMA_SQL).catch((e) => { zipSchemaDone = null; throw e; });
  await zipSchemaDone;
  return pool;
}
const cleanZip = (z) => { const m = String(z || "").trim().match(/^(\d{5})/); return m ? m[1] : ""; };

export async function seedShopAllowedZips(zips = []) {
  const pool = await zipPool();
  const meta = (await pool.query(`SELECT seeded_at FROM shop_allowed_zips_meta WHERE id = 1`)).rows[0];
  if (meta?.seeded_at) return false;
  for (const z of zips.map(cleanZip).filter(Boolean)) {
    await pool.query(`INSERT INTO shop_allowed_zips (zip, added_by) VALUES ($1, 'initial list (SHOP_ALLOWED_ZIPS)') ON CONFLICT (zip) DO NOTHING`, [z]);
  }
  await pool.query(`INSERT INTO shop_allowed_zips_meta (id, seeded_at) VALUES (1, NOW()) ON CONFLICT (id) DO UPDATE SET seeded_at = NOW()`);
  return true;
}
export async function listShopAllowedZips() {
  const pool = await zipPool();
  const rows = (await pool.query(`SELECT zip, label, added_by, added_at FROM shop_allowed_zips ORDER BY zip`)).rows;
  return rows.map((r) => ({ zip: r.zip, label: r.label || "", addedBy: r.added_by || "", addedAt: r.added_at?.toISOString?.() || null }));
}
// "78620, 78737 78746" or an array → the ZIPs actually added.
export async function addShopAllowedZips({ zips, label = "", by = "" }) {
  const list = (Array.isArray(zips) ? zips : String(zips || "").split(/[\s,;]+/)).map((z) => String(z || "").trim()).filter(Boolean);
  const bad = list.filter((z) => !/^\d{5}$/.test(z));
  if (bad.length) throw new Error(`Not 5-digit ZIPs: ${bad.slice(0, 5).join(", ")}`);
  if (!list.length) throw new Error("Enter at least one 5-digit ZIP.");
  const pool = await zipPool();
  const added = [];
  for (const z of [...new Set(list)]) {
    const r = await pool.query(`INSERT INTO shop_allowed_zips (zip, label, added_by) VALUES ($1, $2, $3) ON CONFLICT (zip) DO NOTHING RETURNING zip`,
      [z, String(label || "").trim().slice(0, 60), String(by || "").slice(0, 120)]);
    if (r.rows[0]) added.push(z); // already on the list → left exactly as it was
  }
  return added;
}
// Brands never sold online (the makers don't allow e-commerce). Seeded once
// from the code default; edited on Shop Orders. Entries are ePASS brand
// codes or brand names; matching happens in server.js (shopBrandBlocker).
const BLOCKED_BRAND_SCHEMA_SQL = `
CREATE TABLE IF NOT EXISTS shop_blocked_brands (
...
CREATE TABLE IF NOT EXISTS shop_blocked_brands_meta (
  id INT PRIMARY KEY DEFAULT 1 CHECK (id = 1),
  seeded_at TIMESTAMPTZ
);
`;
let brandSchemaDone = null;
async function blockedBrandPool() {
  const pool = await getReadyPool();
  if (!brandSchemaDone) brandSchemaDone = pool.query(BLOCKED_BRAND_SCHEMA_SQL).catch((e) => { brandSchemaDone = null; throw e; });
  await brandSchemaDone;
  return pool;
}
export const brandEntryKey = (v) => String(v || "").toUpperCase().replace(/[^0-9A-Z]/g, "");
export async function seedShopBlockedBrands(entries = [], label = "") {
  const pool = await blockedBrandPool();
  const meta = (await pool.query(`SELECT seeded_at FROM shop_blocked_brands_meta WHERE id = 1`)).rows[0];
  if (meta?.seeded_at) return false;
  for (const e of entries) {
    const k = brandEntryKey(e); if (!k) continue;
    await pool.query(`INSERT INTO shop_blocked_brands (key, entry, label, added_by) VALUES ($1, $2, $3, 'initial list') ON CONFLICT (key) DO NOTHING`, [k, String(e).trim().slice(0, 40), String(label || "").slice(0, 60)]);
  }
  await pool.query(`INSERT INTO shop_blocked_brands_meta (id, seeded_at) VALUES (1, NOW()) ON CONFLICT (id) DO UPDATE SET seeded_at = NOW()`);
  return true;
}
export async function listShopBlockedBrands() {
  const pool = await blockedBrandPool();
  const rows = (await pool.query(`SELECT key, entry, label, added_by, added_at FROM shop_blocked_brands ORDER BY entry`)).rows;
  return rows.map((r) => ({ key: r.key, entry: r.entry, label: r.label || "", addedBy: r.added_by || "", addedAt: r.added_at?.toISOString?.() || null }));
}
// "SZ, Sub-Zero, WOLF" or an array -> the entries actually added.
export async function addShopBlockedBrands({ entries, label = "", by = "" }) {
  const list = (Array.isArray(entries) ? entries : String(entries || "").split(/[,;\n]+/)).map((e) => String(e || "").trim()).filter(Boolean);
  if (!list.length) throw new Error("Enter a brand name or ePASS brand code.");
  const bad = list.filter((e) => brandEntryKey(e).length < 2);
  if (bad.length) throw new Error(`Too short to match a brand: ${bad.slice(0, 5).join(", ")}`);
  const pool = await blockedBrandPool();
  const added = [];
  for (const e of list) {
    const r = await pool.query(`INSERT INTO shop_blocked_brands (key, entry, label, added_by) VALUES ($1, $2, $3, $4) ON CONFLICT (key) DO NOTHING RETURNING entry`,
      [brandEntryKey(e), e.slice(0, 40), String(label || "").trim().slice(0, 60), String(by || "").slice(0, 120)]);
    if (r.rows[0]) added.push(e);
  }
  return added;
}
export async function removeShopBlockedBrand(entry) {
  const k = brandEntryKey(entry);
  if (!k) throw new Error("Nothing to remove.");
  const pool = await blockedBrandPool();
  return (await pool.query(`DELETE FROM shop_blocked_brands WHERE key = $1`, [k])).rowCount > 0;
}

export async function removeShopAllowedZip(zip) {
  const z = cleanZip(zip);
  if (!z) throw new Error("Not a 5-digit ZIP.");
  const pool = await zipPool();
  return (await pool.query(`DELETE FROM shop_allowed_zips WHERE zip = $1`, [z])).rowCount > 0;
}
