...
// Profile edits from the storefront's account panel. The route validates
// field contents (including the delivery-ZIP gate) before calling this.
export async function updateShopperProfile({ token, firstName, lastName, email, phone, preferredContact, address }) {
  const shopper = await getShopperByToken(token);
  if (!shopper) return null;
  const pool = await getReadyPool();
  const result = await pool.query(
...
export async function saveShopInventorySnapshot({ serials, types, written, sourceFile, uploadedBy }) {
  const pool = await getReadyPool();
  ...
  await pool.query(
    `INSERT INTO shop_inventory_snapshot (id, serials, serial_types, serial_written, source_file, uploaded_by, uploaded_at)
     ...
     ON CONFLICT (id) DO UPDATE SET
       ...
       uploaded_at = NOW()`,
    [JSON.stringify(clean), JSON.stringify(cleanTypes), JSON.stringify(cleanWritten), String(sourceFile || "").slice(0, 200), String(uploadedBy || "").toLowerCase()]
  );
  return { count: clean.length, typedCount: Object.keys(cleanTypes).length, writtenCount: Object.keys(cleanWritten).length };
}
