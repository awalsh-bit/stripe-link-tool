...
// The invoice number (ePASS's name for the sales order number) sits directly
// under the "Invoice Number" header box: R00015347 / S00062359 style.
function extractSalesOrder(rows) {
  let headerCell = null;
  let headerY = null;
  for (const row of rows) {
    const cell = row.cells.find((c) => /^Invoice\s*Number$/i.test(c.str));
    if (cell) {
      headerCell = cell;
      headerY = row.y;
      break;
  ...
  try {
    for (let pageNum = 1; pageNum <= doc.numPages; pageNum += 1) {
      ...
      if (pageNum === 1) {
        ...
        salesOrder = extractSalesOrder(rows);
      }
