...
// Stock plan = ROUNDUP(trailing sales × months / window). Forecast = plan − QOH − QOO.
...
// ttm_months: the trailing sales window. Andrew (2026-09-16) wants six —
// the stock plan annualizes whatever the window is (sales × plan ÷ window).
// tl_discount_pct: the FTL 126 price is wholesale less this percent (3%,
// Andrew 2026-09-17), so regular (wholesale) cost = TL cost ÷ (1 − pct/100)
// unless the catalog carries an explicit regular cost for that model.
export const DEFAULT_SETTINGS = { stock_plan_months: 6, truck_units: 126, ttm_months: 6, tl_discount_pct: 3 };

export function regularCostFor(m, settings = {}) {
  if (m.regularCost != null) return { regularCost: m.regularCost, regularDerived: false };
  const pct = Number(settings.tl_discount_pct);
  const rate = Number.isFinite(pct) && pct > 0 && pct < 100 ? pct / 100 : DEFAULT_SETTINGS.tl_discount_pct / 100;
  if (m.tlCost == null) return { regularCost: null, regularDerived: true };
  return { regularCost: Math.round((m.tlCost / (1 - rate)) * 100) / 100, regularDerived: true };
}
...
// Row shape: [family, note, model, kind, tlCost, qoo (the template's column
// L — units on order, deducted from the forecast), aliases]. The cash
// program columns are parked (Andrew, 2026-09-16: "show cash can be later").
export const SEED_MODELS = [
  // CLASSIC WASH
  ["CLASSIC WASH", "WHITE", "TC5003WN", "washer", 1021, 14],
  ...
  // PERFECT WASH 7
  ["PERFECT WASH 7", "WHITE", "TR7006WN", "washer", 1021, 21, ["TR7003WN"]],
  ["PERFECT WASH 7", "WHITE ELEC", "DR7004WE", "dryer", 1021],
  ["PERFECT WASH 7", "WHITE GAS", "DR7004WG", "dryer", 1089],
  ["PERFECT WASH 7", "BLACK", "TR7006BN", "washer", 1089, null, ["TR7003BN"]],
  ["PERFECT WASH 7", "BLACK ELEC", "DR7004BE", "dryer", 1089, 4],
  ["PERFECT WASH 7", "BLACK GAS", "DR7004BG", "dryer", 1157],
  // PERFECT WASH 5
  ["PERFECT WASH 5", "WHITE", "TR5006WN", "washer", 953, null, ["TR5003WN"]],
  ["PERFECT WASH 5", "WHITE ELEC", "DR5004WE", "dryer", 953],
  ["PERFECT WASH 5", "WHITE GAS", "DR5004WG", "dryer", 1021],
  // PERFECT WASH 3
  ["PERFECT WASH 3", "WHITE", "TR3006WN", "washer", 885, null, ["TR3003WN"]],
  ...
  ["LAUNDRY CENTER", "WHITE ELEC", "SF7008WE", "stacked", 2995, 4, ["SF7007WE"]],
  ["LAUNDRY CENTER", "BLACK ELEC", "SF7008BE", "stacked", 3132, null, ["SF7007BE"]],
  ["LAUNDRY CENTER", "BLACK GAS", "SF7008BG", "stacked", 3200, null, ["SF7007BG"]],
  ["LAUNDRY CENTER", "WHITE GAS", "SF7008WG", "stacked", 3064, null, ["SF7007WG"]],
  // FRONT LOAD
  ["FRONT LOAD", "WHITE LH", "FF7011WN", "washer", 1497, 8, ["FF7009WN"]],
  ["FRONT LOAD", "WHITE RH", "FF7012WN", "washer", 1497, null, ["FF7010WN"]],
  ["FRONT LOAD", "WHITE ELEC", "DF7004WE", "dryer", 1497],
  ["FRONT LOAD", "WHITE GAS", "DF7004WG", "dryer", 1565],
  ["FRONT LOAD", "BLACK LH", "FF7011BN", "washer", 1565, null, ["FF7009BN"]],
  ["FRONT LOAD", "BLACK RH", "FF7012BN", "washer", 1565, 1, ["FF7010BN"]],
...
async function seedIfEmpty(pool) {
  ...
  if (n) return;
  let sort = 0;
  for (const [family, note, model, kind, tlCost, qoo, aliases] of SEED_MODELS) {
    await pool.query(
      `INSERT INTO sq_tl_models (model, family, note, kind, tl_cost, qoo, qoo_note, aliases, sort) VALUES ($1,$2,$3,$4,$5,$6,$7,$8::jsonb,$9) ON CONFLICT (model) DO NOTHING`,
      [model, family, note, kind, tlCost ?? null, qoo ?? 0, qoo ? "from the SQ TL Template" : "", JSON.stringify(aliases || []), sort++]
    );
  }
}
...
export async function setSqSetting(key, value) {
  ...
  if (key === "tl_discount_pct" && n >= 100) throw new Error("Truckload discount must be under 100%.");
  const pool = await getReadyPool();
  ...
  const r = await pool.query(
    `INSERT INTO sq_tl_models (model, family, note, kind, tl_cost, regular_cost, cash_qty, cash_dollars, aliases, sort, active, updated_at)
     ...
     ON CONFLICT (model) DO UPDATE SET family = EXCLUDED.family, note = EXCLUDED.note, kind = EXCLUDED.kind, tl_cost = EXCLUDED.tl_cost, regular_cost = EXCLUDED.regular_cost,
       cash_qty = COALESCE(EXCLUDED.cash_qty, sq_tl_models.cash_qty), cash_dollars = COALESCE(EXCLUDED.cash_dollars, sq_tl_models.cash_dollars),
       aliases = EXCLUDED.aliases, sort = EXCLUDED.sort, active = EXCLUDED.active, updated_at = NOW()
     RETURNING *`,
...
export function computeSqRow(m, sales, inv, settings, qty) {
  ...
  const window = Number(settings.ttm_months) || DEFAULT_SETTINGS.ttm_months;
  const ttm = sales?.ttm || 0;
  // Template: ROUNDUP(TTM × months / 12). Generalized to the window in use.
  const stockPlan = Math.ceil((ttm * months) / window);
  const qoh = inv?.qoh || 0;
  ...
  const { regularCost, regularDerived } = regularCostFor(m, settings);
  const regularLineCost = regularCost != null ? Math.round(regularCost * q * 100) / 100 : null;
  const savings = m.tlCost != null && regularCost != null ? Math.round((regularCost - m.tlCost) * q * 100) / 100 : null;
  return { stockPlan, qoh, written: inv?.written || 0, oldestReceived: inv?.oldest || "", qoo: m.qoo || 0, forecast, ttm, lastYear: sales?.lastYear || 0, thisYear: sales?.thisYear || 0, qty: q, truckUnits: units, lineCost, regularCost, regularDerived, regularLineCost, savings };
}
...
export async function buildSqBoard() {
  ...
  const [sales, inv] = await Promise.all([sqSalesByModel([...keys], { ttmMonths: Number(settings.ttm_months) || DEFAULT_SETTINGS.ttm_months }), sqInventoryByModel([...keys])]);
  ...
  const totals = {
    ...
    regularCost: Math.round(active.reduce((a, r) => a + (r.regularLineCost || 0), 0) * 100) / 100,
