...
// STRIPE_API_BASE (e.g. http://127.0.0.1:4470) points the client at a mock
// Stripe server for integration tests. Unset in production — the default
// api.stripe.com wiring is untouched.
function stripeOptions() {
  const base = String(process.env.STRIPE_API_BASE || "").trim();
  if (!base) return {};
...
// Deterministic JSON serialization: sorted object keys, undefined values
// dropped from objects (mirroring JSON.stringify) and rendered as null
// inside arrays (also mirroring JSON.stringify).  This is what makes the
// idempotency key stable across calls regardless of property insertion
...
// Derives a Stripe idempotency key from the FULL request payload that
// will be sent to Stripe.  Use this in preference to the older
// createStripeIdempotencyKey above: hashing the entire payload makes it
// structurally impossible for an "unhashed" field to drift between a
// first and second submission, which is what causes Stripe to reject
// retries with "Keys for idempotent requests can only be used with the
// same parameters they were first used with."
export function createStripeIdempotencyKeyFromPayload(prefix, payload) {
  const serialized = stableStringify(payload);
  const digest = crypto
    .createHash("sha256")
    .update(`${prefix}|${serialized === undefined ? "" : serialized}`)
    .digest("hex")
    .slice(0, 32);
  return `${prefix}-${digest}`;
}
