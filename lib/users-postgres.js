...
export async function setUserStatus(userId, status, actorUserId = null) {
...
export async function setUserExecutive(userId, isExecutive, actorUserId = null) {
  const pool = await getReadyPool();
  const client = await pool.connect();
...
export async function setUserPagePermissions(userId, changes, actorUserId = null) {
...
async function insertAuditRow(client, { actorUserId = null, action, targetUserId = null, detail = {} }) {
  await client.query(
    `INSERT INTO access_audit_log (actor_user_id, action, target_user_id, detail)
     VALUES ($1, $2, $3, $4::jsonb)`,
    [actorUserId, String(action), targetUserId, JSON.stringify(detail || {})]
  );
}
