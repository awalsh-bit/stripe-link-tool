...
// A directory entry's email changed (a typo fixed, a rename): move the
// login account with it, so the invite goes to the right address and the
// account doesn't sit orphaned on the old one. Outstanding invite/verify
// tokens issued to the old address are invalidated; the caller re-invites.
export async function updateUserEmail(userId, email) {
  const normalized = normalizeEmail(email);
  if (!normalized) throw new Error("A valid email is required.");
  const pool = await getReadyPool();
  const clash = await pool.query(`SELECT id FROM app_users WHERE email = $1 AND id <> $2`, [normalized, userId]);
  if (clash.rows[0]) throw new Error(`Another login account already uses ${normalized}.`);
  const result = await pool.query(`UPDATE app_users SET email = $2, updated_at = NOW() WHERE id = $1 RETURNING *`, [userId, normalized]);
  await pool.query(`UPDATE auth_tokens SET consumed_at = NOW() WHERE user_id = $1 AND consumed_at IS NULL AND kind IN ('invite','verify')`, [userId]).catch(() => {});
  return result.rows[0] || null;
}
...
// Remove a never-activated account entirely (stale invites / abandoned
// registrations). Callers must enforce that the account is still in the
// invited or pending_verification state — active or deactivated accounts are
// history-bearing and are deactivated, never deleted.
export async function deleteUserAccount(userId) {
  const pool = await getReadyPool();
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    // Clear non-cascading back-references before the row goes.
    await client.query(`UPDATE app_users SET created_by = NULL WHERE created_by = $1`, [userId]);
    await client.query(`UPDATE user_page_permissions SET updated_by = NULL WHERE updated_by = $1`, [userId]);
    const result = await client.query(`DELETE FROM app_users WHERE id = $1 RETURNING id`, [userId]);
    await client.query("COMMIT");
