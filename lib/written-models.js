...
export function parseWrittenModelsFromEpass(bundle) {
  ...
  for (const r of ds("open-order-lines")) {
    ...
    // What ePASS's own Written Models report leaves out (derived 2026-09-21
    // by matching the 9/18 OE-04 against this feed, 491 of 536 lines, every
    // miss explained by a PO or delivery since): a line already on a purchase
    // order is ordered, and a D1 invoice (written, not yet released) is not
    // ready to order. Both stay in the ePASS tables for the inventory view.
    if (str(r, "POCode")) { skipped.ordered++; continue; }
    if (str(inv, "JobStatusCode").toUpperCase() === "D1") { skipped.d1++; continue; }
    const rec = modelRec(model);
    ...
    const line = {
      model: rec.model, key: rec.key, qty, loc: str(r, "LocationCode"), po: str(r, "POCode"), poDate: dateOf(get(r, "PODateStamp")),
  ...
  for (const m of models) {
    m.written = lines.filter((l) => l.key === m.key).reduce((a, l) => a + (l.qty || 0), 0);
...
export async function buildWrittenModelsBoard({ month = "", through = "" } = {}) {
  ...
  const supplierOptions = [...new Set([...supplierRows, ...models.map((m) => m.supplierName || m.supplierCode || ""), ...Object.values(settings.supplierMap || {}), ...Object.values(settings.modelSupplier || {})].map((x) => String(x || "").trim()).filter(Boolean))].sort((a, b) => a.localeCompare(b));
  ...
  const supplierFor = (key, item, rec) => {
    ...
    if (rec?.supplierName || rec?.supplierCode) return { supplier: rec.supplierName || rec.supplierCode, source: "epass" };
  ...
  const modelRows = models.map((rec) => {
    ...
    // Short = written minus what can actually cover it: units on hand that
    // aren't promised to another invoice, and stock on order (not a PO cut
    // for someone else). The OE-04 snapshot has neither split, so it falls
    // back to plain QOH / QOO as before.
    const proposed = Math.max(0, written - (rec.freeQoh ?? rec.qoh ?? 0) - (rec.stockQoo ?? rec.qoo ?? 0));
    ...
    return {
      ...
      spokenFor: rec.spokenFor || [], proposed, stdCost: std, freeQoh: rec.freeQoh ?? null, specialQoo: rec.specialQoo ?? null,
  ...
  return { sources, settings, supplierOptions, months, month: m, through: th, throughDefault: defaultThrough, suppliers, totals, warnings: Array.isArray(s.warnings) ? s.warnings : [] };
}
