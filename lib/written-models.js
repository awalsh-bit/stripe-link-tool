...
export function parseWrittenModelsFromEpass(bundle) {
  ...
  // Serials on hand already written to an open invoice.
  for (const r of ds("open-order-serials")) {
    if (bool(get(r, "Taken")) || bool(get(r, "Returned"))) continue;
    const key = normModel(str(r, "ModelCode"));
    const rec = byModel.get(key);
    if (!rec) continue;
    const invCode = str(r, "InvoiceCode").toUpperCase();
    const inv = invoices.get(invCode);
    rec.spokenFor.push({ invoice: text(invCode, 20), serial: str(r, "SerialCode"), customer: inv ? [str(inv, "SoldToFirstName"), str(inv, "SoldToLastName")].filter(Boolean).join(" ").slice(0, 80) : "", qty: 1, status: str(r, "Status"), deliveryDate: inv ? dateOf(get(inv, "ScheduleDate")) : "", daysInInventory: null, location: str(r, "LocationCode") });
  }

  for (const m of models) m.written = lines.filter((l) => l.key === m.key).reduce((a, l) => a + (l.qty || 0), 0);
  ...
  return { reportDate, startDate: "", endDate: "", lines, models, warnings, source: "epass-odbc" };
}
