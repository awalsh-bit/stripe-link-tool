...
export function parseWrittenModelsFromEpass(bundle) {
  ...
  for (const m of models) {
    ...
    if (m.units) {
      // Free to rotate = on hand, not promised to an invoice, and an ordinary
      // (ALL) unit. Showroom / open-box / manager-special / service / RTV units
      // are "exclusive reserve": they can be sold, but only by choice — they
      // never cover a written line on their own (Andrew, 9/22).
      m.units.sort((a, b) => (a.received || "9").localeCompare(b.received || "9") || a.serial.localeCompare(b.serial)); // oldest first
      const isAll = (u) => !u.serialType || u.serialType.toUpperCase() === "ALL";
      for (const u of m.units) u.exclusive = !u.writtenTo && !isAll(u);
      if (m.qoh == null) m.qoh = m.units.length;
      m.freeQoh = m.units.filter((u) => !u.writtenTo && isAll(u)).length;
      m.exclusiveQoh = m.units.filter((u) => u.exclusive).length;
    }
...
// kind (Andrew, 9/22): "ordered" (green cart — the PO is cut, off the
// report), "later" (yellow arrow — stays on the report, muted, ordering
// deferred), "rejected" (red message — purchasing can't or won't order it
// as written; the note goes to the salesperson as a dashboard flag + email,
// the line stays visible until they fix it or purchasing changes its mind).
export const LINE_ACTIONS = ["ordered", "later", "rejected"];
export async function setWrittenLineHandled(lineId, handled, by = "", { kind = "ordered", note = "" } = {}) {
  const id = String(lineId || "").slice(0, 120);
  if (!id) throw new Error("Line id is required.");
  const k = LINE_ACTIONS.includes(kind) ? kind : "ordered";
  const pool = await getReadyPool();
  const cur = (await pool.query(`SELECT value FROM written_models_settings WHERE key = 'handled_lines'`)).rows[0]?.value || {};
  const next = { ...cur };
  if (handled) next[id] = { at: new Date().toISOString(), by: String(by || "").slice(0, 120), kind: k, ...(note ? { note: String(note).slice(0, 600) } : {}) }; else delete next[id];
...
export async function buildWrittenModelsBoard({ month = "", through = "" } = {}) {
  ...
  // Every supplier name the board shows is NetSuite's. Whatever source a name
  // came from — a per-model override typed before the vendor list had its
  // numbers, the brand map ("Sub-Zero Group"), an ePASS code (ALMO, WESTYE) —
  // it is snapped to the NetSuite vendor whose name starts with the same word.
  // Andrew, 9/22: "Almo" was landing in its own group beside "0015 Almo
  // Corporation", and hand-typed names had no vendor number.
  const NOISE = new Set(["corporation", "corp", "inc", "group", "usa", "home", "appliances", "appliance", "distribution", "distributing", "manufacturing", "brands", "electronics", "south", "central", "co", "company", "parts", "part", "llc"]);
  const keyWord = (name) => { const toks = String(name || "").toLowerCase().replace(/^\d{3,}\s+/, "").replace(/[.,&()]/g, " ").split(/[\s/]+/).filter((t) => t && !NOISE.has(t)); return toks[0] || ""; };
  const EPASS_ALIAS = { westye: "sub-zero", wolf: "sub-zero", subzero: "sub-zero", "sub": "sub-zero", gec: "ge", geappliances: "ge", lgeus: "lg", wp001: "whirlpool", bshparts: "bsh", compbr: "composition", milestone: "milestone", "fisherpayk": "fisher" };
  const nsByKey = new Map(); for (const v of supplierRows) { const k = keyWord(v); if (k && !nsByKey.has(k)) nsByKey.set(k, v); }
  const canonical = (name) => {
    const raw = String(name || "").trim(); if (!raw) return "";
    if (supplierRows.includes(raw)) return raw;
    let k = keyWord(raw); if (EPASS_ALIAS[k]) k = EPASS_ALIAS[k]; if (EPASS_ALIAS[raw.toLowerCase().replace(/\s+/g, "")]) k = EPASS_ALIAS[raw.toLowerCase().replace(/\s+/g, "")];
    return nsByKey.get(k) || raw;
  };
  ...
  for (const code of epassCodes.keys()) {
    ...
    if (manual) { epassMap.set(code, { supplier: canonical(manual), source: "settings" }); continue; }
    ...
    if (DEFAULT_EPASS_SUPPLIER_MAP[code]) { epassMap.set(code, { supplier: canonical(DEFAULT_EPASS_SUPPLIER_MAP[code]), source: "default" }); continue; }
    // No seed, nothing learned: the code or its ePASS name may still spell the vendor (ALMO, MIELE PART).
    const guess = canonical(code); const guess2 = canonical(epassCodes.get(code)?.name || "");
    if (supplierRows.includes(guess)) epassMap.set(code, { supplier: guess, source: "name" });
    else if (supplierRows.includes(guess2)) epassMap.set(code, { supplier: guess2, source: "name" });
  }
  ...
  const supplierFor = (key, item, rec) => {
    ...
    if (override) return { supplier: canonical(override), source: "override" };
    ...
    if (brand && settings.supplierMap[brand]) return { supplier: canonical(settings.supplierMap[brand]), source: "brand" };
    if (rec?.supplierName || code) { const c = canonical(rec.supplierName || code); return { supplier: c, source: supplierRows.includes(c) ? "epass-map" : "epass" }; } // unmapped ePASS supplier — flagged on the page
...
export async function getInventoryPosition(modelKey) {
  ...
  if (Array.isArray(rec?.units)) {
    ...
    units = rec.units.map((u) => ({
      ...
      location: u.location || "", bin: u.bin || "", po: u.po || "", exclusive: !!u.exclusive
    }));
    units.sort((a, b) => (a.received || "9").localeCompare(b.received || "9") || a.serial.localeCompare(b.serial)); // oldest first
  } else try {
  ...
  return {
    ...
    summary: {
      ...
      spokenFor: spokenUnits, free: rec?.freeQoh ?? (rec?.qoh != null ? Math.max(0, rec.qoh - spokenUnits) : null), exclusive: rec?.exclusiveQoh ?? null,
