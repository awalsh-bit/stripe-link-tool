...
_Current prototype generation: v0.9.16_

## Since v0.9.14, in one place (read this before the merge)

- **v0.9.15 -- the server.** Loopback is the default bind; `--lan` refuses to
  start without a passcode (`--set-passcode`, salted PBKDF2 in
  `.wilson-passcode`, gitignored). Every page and every API POST is behind a
  12-hour session cookie. New: `POST /api/photos` and `GET /api/photos`,
  `assets/photo-sync.js`, `photo-store/` (gitignored).
- **v0.9.16 -- what a customer is allowed to be told.** The lifecycle stage
  "Replacement Planning" is gone (it fired on age alone against a draft life
  figure); the "replacement is worth planning for" and "worth budgeting for"
  postures are now statements of fact; an HVAC report no longer claims age was
  scored into it; an unmeasurable system prints "Not scored" with its reason
  rather than 0%; a checkpoint marked not-applicable no longer becomes a
  customer-facing follow-up; "Above range" readings are flagged rather than
  styled as unremarkable; expected-life figures carry the word "draft".
- **v0.9.16 -- the migration.** `sql/maintenance_migration_v09.sql` had been
  generated from a config predating the HVAC work: 11 templates instead of 15,
  and zero HVAC protocols, brand tiers or expected-life rows.
  `_qa/verify-sql-migration.py` now compares the seed against `plan-config.js`
  live and fails on drift; `_qa/verify-protocol-parity.py` now exercises the
  `hvac` group (which is how this went unnoticed) and the SQL view gained the
  HVAC resolution arm the JS resolver always had.
- **QA:** `bash _qa/run_all.sh` runs 15 suites / 372 checks. Two new ones:
  `verify-report-honesty.py` (what a customer may be told) and
  `verify-field-ergonomics.py` (tap targets, contrast, label association,
  nameplate carry-forward -- all measured in a browser).
_
...
## 32. Prototype file map (v0.9.1 -- see CHANGELOG for files added since)
...
report-view.html            Locked customer appliance health-report presentation (one appliance)
visit-report.html           Compiled whole-visit maintenance review (one stop, every appliance)

assets/photo-store.js       Field photographs in IndexedDB (offline, downscaled, per-checkpoint)
assets/hvac-performance.js  HVAC derived readings, nameplate scoring, and the planning horizon
assets/paginate.js          Flows long report sections across sheets instead of clipping them
assets/offline.js           Offline status, and honest wording about what is held on the device
sw.js                       Service worker: app shell precache, versioned per release
app.webmanifest             Installs to the home screen
assets/plan-config.js       ...also WILSON_AGE: where an appliance's age came from
(retired v0.9.23 — quotes are built on appliance-signup.html)
...
assets/plan-config.js       Business-rule engine: pricing constants, all four plan
                            ...
                            taxonomies, all fifteen checkpoint sets, scoring weights,
...
(retired v0.9.23 — see assets/appliance-builder.js, buildEnrollmentPayload)
...
_qa/verify-invoice-parser.py   Invoice parser regression suite
_qa/invoice_fixtures.py     Synthetic invoice PDFs (no customer PII)
...
(removed)                    screenshots_v03/ deleted in v0.9.46 — two images carried real customer PII from a live invoice import
...
- Outdoor grills are inspected for function and safety only. Cleaning is never performed, on any visit or any plan; condition is rated because it feeds the health score, and reported for the customer to act on.
- The grill scope exclusion is visible on the picker tile, in the enrollment summary, and on the technician's protocol.
- A WashTower / laundry center enrolls as one tile and becomes two maintained appliances, priced as two, with the two-appliance count disclosed before the tile is clicked.
- Appliance icons resolve from category config, never from a hardcoded map.
...
- Payment exceptions appear at top of internal work queue, which is one ordered list rather than parallel columns.
- Every count on the dashboard appears exactly once, and clicking it filters the queue.
- No operating table exceeds five columns; secondary detail folds into the cell it belongs to.
- The first actionable item sits on the first screen of the dashboard.
...
- Rating a check is what completes it; there is no separate performed step.
- No check ever arrives pre-rated. A rating exists only because a technician chose it.
- "Not applicable to this unit" is a distinct state from "not started" and is excluded from scoring.
- The technician chooses which check to work; the app never imposes an order.
...
- An imported WashTower becomes two assets, each on its own washer/dryer protocol.
- One unreadable file never discards the other files in an invoice upload.
- Field data is the sole source for customer health reports.
- A health report never guesses which report was meant; an unknown id refuses rather than showing another household's data.
- Trend charts never put two measures on one pair of axes, and never colour a delta by its sign alone.
- Decline detection has exactly one implementation, shared by every surface that shows it.
- A customer changing their own set point is never reported as a declining appliance.
- Declining appliances are surfaced separately from the visit queue: the queue is today's work.
- Every decline signal states the numbers behind it and is never phrased as a diagnosis.
- Replacement is never suggested for an appliance with remaining expected life and an identified serviceable cause.
- The tool invents no repair or replacement costs; economics appear only from figures Wilson supplies.
- Seeded demo scores are derived from vitals with the field tool's own weighting, never chosen directly.
...
13. ~~Confirm that a combined WashTower/laundry-center bills as washer + dryer.~~ **Confirmed by Wilson and implemented in v0.9.5** — one enrollment tile, two maintained appliances, disclosed before the click. Original note retained for context: This is no longer an open decision: the shipped system already treats it as two maintained assets — invoice import splits it, and each half is priced, scheduled and inspected independently (v0.9.4). Wilson need only confirm the billing intent; if it should bill as ONE appliance the change is in `invoice-import.js`.
14. ~~Decide whether outdoor cooking equipment is a maintained asset class.~~ **Resolved in v0.9.5:** grills are enrollable on a functional-only protocol; Wilson never cleans them. Still open underneath it: whether other outdoor equipment (pizza ovens, outdoor refrigeration beyond the existing refrigeration category, power burners) needs its own treatment, and whether a grill's condition rating should cap its health score rather than merely lower it.
...
10. Wilson dashboard/customer-facing screenshots — visual-language reference. **Removed in v0.9.46: two carried real customer PII (invoice-import and invoice-draft captures made against a real invoice). Any future reference captures must use seeded demo households only.**
