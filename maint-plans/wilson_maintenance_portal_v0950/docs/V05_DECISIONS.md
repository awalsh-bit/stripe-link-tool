...
Age is required separately from the 3-5 current-condition health checkpoints. All checkpoints and a serial-tag photo are required before an appliance can be completed. The prototype stores photo names/counts only **(superseded in v0.9.12: real images are stored, and in v0.9.15: uploaded to the serving machine)**; production should upload image files to secure object storage.
