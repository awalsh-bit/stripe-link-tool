...
class RawLineItem:
    ...
    extended_price: str = ""
    invoice_date: str = ""
    invoice_date_confidence: str = "none"
...
def _invoice_date(lines: list[str], invoice_number: str, joined: str) -> tuple[str, str]:
    """Return (date, confidence).

    The date matters more than it looks: it is the only documented evidence of
    how old an appliance is, and age carries a quarter of every health score.
    Until now it was read as "the first d/d/yyyy in the first 1,200 characters"
    and then thrown away, so nothing depended on it being right.

    On a Wilson invoice the date is printed directly beneath the invoice number
    in the top-right header block, so anchoring to the invoice number is a much
    stronger read than taking whatever date appears first. The loose scan stays
    as a fallback, and says so, because a wrong install year is worse than a
    missing one: a missing one is reported as unknown, a wrong one silently
    moves the score.
    """
    if invoice_number:
        for index, line in enumerate(lines):
            if invoice_number.upper() not in line.upper():
                continue
    ...
    match = DATE_RE.search(joined[:1200])
    if match:
        return match.group(0), "low"
    return "", "none"


def _install_year(invoice_date: str) -> int | None:
    """The sale year, which for an appliance Wilson sold and installed is the
    install year within a few weeks. Not a guess -- but not the same claim as an
    installation record either, which is why the source is labelled as the
    invoice rather than as an install date."""
    match = DATE_RE.fullmatch(invoice_date.strip()) if invoice_date else None
    if not match:
        return None
...
def _extract_raw_items(pdf_bytes: bytes, source_file: str) -> tuple[list[RawLineItem], dict[str, Any]]:
    ...
    invoice_date, date_confidence = _invoice_date(all_layout, invoice_number, joined)
    metadata: dict[str, Any] = {
        "invoiceNumber": invoice_number,
        "invoiceDate": invoice_date,
        "invoiceDateConfidence": date_confidence,
        "installYear": _install_year(invoice_date),
        "contact": contact,
    ...
    def finalize() -> None:
        ...
        items.append(RawLineItem(
            ...
            invoice_date_confidence=date_confidence,
        ))
        current = None
...
#
# Cayden, testing a real customer-pickup invoice: "it did not recognize the dryer
# and determined it was an install/ support line. probably because the demo
# invoices included install. this example is a customer pickup where only product
# existed on the invoice."
#
# He had the symptom right and the cause was one layer down. The invoice's terms
# block was being appended to the LAST product's description, and one of those
# terms lines reads "Please refer to owners manual/installation guides" -- so the
# exclusion phrase "install" matched, and the dryer was dropped as a support line.
#
# There WAS a guard for this. It read:
#
#     stripped.startswith("*Above order")
#
# and pypdf's layout extraction emits that line as "*      Above order confirmed"
# -- an asterisk, then six spaces. The guard never fired on a real invoice. On
# invoices with an install section the damage was invisible, because the footer
# landed on a line that was already being excluded.
#
# So: markers are matched against WHITESPACE-COLLAPSED text, and the list covers
# every line this invoice's footer actually produces rather than the one line
# somebody happened to notice first.
FOOTER_MARKERS = (
...
def _is_footer_line(line: str) -> bool:
    """True once the line belongs to the invoice's terms/totals block.

    Collapses runs of whitespace first: layout-mode extraction pads these lines
    with alignment spaces, and matching against the raw string is how the
    previous guard came to never fire.
    """
    blob = _clean_space(line).lower().lstrip("* ").strip()
...
def _is_excluded(item: RawLineItem) -> tuple[bool, str]:
    ...
    for phrase in EXCLUDE_PHRASES:
        # WORD BOUNDARIES, not a bare substring. "install" matched "installation
        # guides" in an invoice footer and dropped a real dryer; the footer no
        # longer reaches this blob, but a product legitimately described as
        # "installation-ready" or "pre-installed" would have hit it too. The
        # forms Wilson's invoices actually use -- install, installs, installed,
        # installation -- are each in the list, so precision here costs no
        # coverage.
...
def _classify(item: RawLineItem) -> dict[str, Any] | None:
    ...
    if any(term in blob for term in ("washtower", "washer/dryer combo", "washer dryer combo", "laundry center")):
        ...
        confidence = "high"
        needs_review = True
        notes.append("Split into one washer and one dryer maintenance record, each inspected on its own protocol.")
    elif any(term in blob for term in ("outdoor grill", "bbq grill", "barbecue grill", "kamado", "built-in grill", "gas grill", "charcoal grill")):
        ...
        # enrollment on the same category and protocol instead of dropping every
        # grill into the review queue with nowhere valid to go.
        exact_type = "outdoor_grill"
        exact_label = "Outdoor Grill"
        category = "outdoor_grill"
        confidence = "high"
        notes.append("Outdoor grills are inspected for function and safety only; cleaning is never included on any visit.")
    ...
    return {
        ...
        # The documented age input. `installYear` is None when the invoice gave
        # up no readable date, and the importer reports that as unknown rather
        # than substituting something plausible.
        "invoiceDate": item.invoice_date,
        "invoiceDateConfidence": item.invoice_date_confidence,
        "installYear": _install_year(item.invoice_date),
        "price": item.price,
...
def parse_invoice_files(files: Iterable[tuple[str, bytes]]) -> dict[str, Any]:
    # Materialise once: the input is typed Iterable, and the result reads it
    # twice (loop plus sourceFiles), which silently yields an empty sourceFiles
    # list for a generator caller.
    files = list(files)
    ...
    for filename, payload in files:
        # A corrupt, truncated, empty or non-PDF file used to raise straight out
        # of here, which the server turned into a single failed request -- losing
        # every good file uploaded alongside it. Split invoices mean two or three
        # files per import and partial downloads are common, so one bad file must
        # not discard the rest.
        try:
            raw_items, metadata = _extract_raw_items(payload, filename)
        except Exception as exc:  # noqa: BLE001 - any pypdf failure is "unreadable"
            unreadable.append(filename)
            warnings.append(
                f"{filename} could not be read as a PDF invoice and was skipped "
                f"({type(exc).__name__}). Re-download or re-export it and try again."
            )
            continue
    ...
    # Every file unreadable is a failed import, not an empty one -- say so plainly
    # rather than returning a successful result with nothing in it.
    if unreadable and not metadata_records:
        return {
            "ok": False,
            "error": (
                "None of the uploaded files could be read as PDF invoices: "
                + ", ".join(unreadable)
            ),
