/*
Wilson Maintenance Portal - SQL Server bootstrap schema v0.3
...
quotes, Wilson sales-invoice import, NetSuite integration state, and audit history.
...
IF NOT EXISTS (
    SELECT 1 FROM dbo.MaintenanceSchemaVersions WHERE VersionNumber = '0.3'
)
BEGIN
    INSERT INTO dbo.MaintenanceSchemaVersions (VersionNumber, Notes)
    VALUES ('0.3', 'Icon-first customer enrollment and Wilson PDF sales-invoice import staging.');
END;
GO
...
IF COL_LENGTH('dbo.MaintenanceAssets', 'CustomerCategoryCode') IS NULL
    ALTER TABLE dbo.MaintenanceAssets ADD CustomerCategoryCode NVARCHAR(50) NULL;
GO
IF COL_LENGTH('dbo.MaintenanceAssets', 'SourceInvoiceNumber') IS NULL
    ALTER TABLE dbo.MaintenanceAssets ADD SourceInvoiceNumber NVARCHAR(100) NULL;
GO
IF COL_LENGTH('dbo.MaintenanceAssets', 'SourceInvoiceImportLineId') IS NULL
    ALTER TABLE dbo.MaintenanceAssets ADD SourceInvoiceImportLineId BIGINT NULL;
GO
IF COL_LENGTH('dbo.MaintenanceAssets', 'SourceDescription') IS NULL
    ALTER TABLE dbo.MaintenanceAssets ADD SourceDescription NVARCHAR(MAX) NULL;
GO
IF COL_LENGTH('dbo.MaintenanceAssets', 'NeedsClassificationReview') IS NULL
    ALTER TABLE dbo.MaintenanceAssets ADD NeedsClassificationReview BIT NOT NULL
        CONSTRAINT DF_MaintenanceAssets_ClassificationReview DEFAULT (0);
GO
...
/* -------------------------------------------------------------------------
   Wilson sales-invoice import staging
   ------------------------------------------------------------------------- */
IF OBJECT_ID('dbo.MaintenanceInvoiceImportBatches', 'U') IS NULL
BEGIN
    CREATE TABLE dbo.MaintenanceInvoiceImportBatches (
        InvoiceImportBatchId BIGINT IDENTITY(1,1) NOT NULL
            CONSTRAINT PK_MaintenanceInvoiceImportBatches PRIMARY KEY,
        ImportReference NVARCHAR(50) NULL,
        ImportStatus NVARCHAR(40) NOT NULL
            CONSTRAINT DF_MaintenanceInvoiceImportBatches_Status DEFAULT ('Parsed'),
        ParserVersion NVARCHAR(30) NOT NULL,
        InvoiceNumbersJson NVARCHAR(MAX) NULL,
        ShipToName NVARCHAR(255) NULL,
        ShipToPhone NVARCHAR(50) NULL,
        ShipToAddress1 NVARCHAR(255) NULL,
        ShipToCity NVARCHAR(100) NULL,
        ShipToStateCode NVARCHAR(10) NULL,
        ShipToPostalCode NVARCHAR(20) NULL,
        WarningJson NVARCHAR(MAX) NULL,
        CreatedByUserId NVARCHAR(100) NULL,
        CreatedAt DATETIME2 NOT NULL
            CONSTRAINT DF_MaintenanceInvoiceImportBatches_CreatedAt DEFAULT SYSUTCDATETIME(),
        ReviewedByUserId NVARCHAR(100) NULL,
        ReviewedAt DATETIME2 NULL,
        ConvertedHouseholdId INT NULL,
        CONSTRAINT FK_MaintenanceInvoiceImportBatches_Households
            FOREIGN KEY (ConvertedHouseholdId) REFERENCES dbo.MaintenanceHouseholds(HouseholdId)
    );

    CREATE INDEX IX_MaintenanceInvoiceImportBatches_Status
        ON dbo.MaintenanceInvoiceImportBatches(ImportStatus, CreatedAt DESC);
END;
GO

IF OBJECT_ID('dbo.MaintenanceInvoiceImportFiles', 'U') IS NULL
BEGIN
    CREATE TABLE dbo.MaintenanceInvoiceImportFiles (
        InvoiceImportFileId BIGINT IDENTITY(1,1) NOT NULL
            CONSTRAINT PK_MaintenanceInvoiceImportFiles PRIMARY KEY,
        InvoiceImportBatchId BIGINT NOT NULL,
        SourceFileName NVARCHAR(255) NOT NULL,
        SourceStorageKey NVARCHAR(500) NULL,
        SourceSha256 CHAR(64) NULL,
        InvoiceNumber NVARCHAR(100) NULL,
        InvoiceDate DATE NULL,
        PageCount INT NULL,
        SplitReferencesJson NVARCHAR(MAX) NULL,
        ExtractedTextStorageKey NVARCHAR(500) NULL,
        CreatedAt DATETIME2 NOT NULL
            CONSTRAINT DF_MaintenanceInvoiceImportFiles_CreatedAt DEFAULT SYSUTCDATETIME(),
        CONSTRAINT FK_MaintenanceInvoiceImportFiles_Batches
            FOREIGN KEY (InvoiceImportBatchId) REFERENCES dbo.MaintenanceInvoiceImportBatches(InvoiceImportBatchId) ON DELETE CASCADE
    );

    CREATE INDEX IX_MaintenanceInvoiceImportFiles_Invoice
        ON dbo.MaintenanceInvoiceImportFiles(InvoiceNumber, InvoiceImportBatchId);
END;
GO

IF OBJECT_ID('dbo.MaintenanceInvoiceImportLines', 'U') IS NULL
BEGIN
    CREATE TABLE dbo.MaintenanceInvoiceImportLines (
        InvoiceImportLineId BIGINT IDENTITY(1,1) NOT NULL
            CONSTRAINT PK_MaintenanceInvoiceImportLines PRIMARY KEY,
        InvoiceImportFileId BIGINT NOT NULL,
        SourceAreaName NVARCHAR(150) NULL,
        SourceQuantity INT NULL,
        Model NVARCHAR(100) NULL,
        Brand NVARCHAR(100) NULL,
        SerialNumber NVARCHAR(150) NULL,
        SourceDescription NVARCHAR(MAX) NULL,
        SourceClassification NVARCHAR(500) NULL,
        ExactTypeCode NVARCHAR(50) NULL,
        ExactTypeName NVARCHAR(100) NULL,
        CustomerCategoryCode NVARCHAR(50) NULL,
        ConfidenceCode NVARCHAR(20) NULL,
        IncludeInMaintenanceDraft BIT NOT NULL
            CONSTRAINT DF_MaintenanceInvoiceImportLines_Include DEFAULT (1),
        NeedsReview BIT NOT NULL
            CONSTRAINT DF_MaintenanceInvoiceImportLines_Review DEFAULT (0),
        IgnoreReason NVARCHAR(500) NULL,
        UserEditedAreaName NVARCHAR(150) NULL,
        UserEditedQuantity INT NULL,
        UserEditedCustomerCategoryCode NVARCHAR(50) NULL,
        RawLineJson NVARCHAR(MAX) NULL,
        CreatedAssetId INT NULL,
        CreatedAt DATETIME2 NOT NULL
            CONSTRAINT DF_MaintenanceInvoiceImportLines_CreatedAt DEFAULT SYSUTCDATETIME(),
        UpdatedAt DATETIME2 NOT NULL
            CONSTRAINT DF_MaintenanceInvoiceImportLines_UpdatedAt DEFAULT SYSUTCDATETIME(),
        CONSTRAINT FK_MaintenanceInvoiceImportLines_Files
            FOREIGN KEY (InvoiceImportFileId) REFERENCES dbo.MaintenanceInvoiceImportFiles(InvoiceImportFileId) ON DELETE CASCADE,
        CONSTRAINT FK_MaintenanceInvoiceImportLines_Assets
            FOREIGN KEY (CreatedAssetId) REFERENCES dbo.MaintenanceAssets(AssetId)
    );

    CREATE INDEX IX_MaintenanceInvoiceImportLines_Review
        ON dbo.MaintenanceInvoiceImportLines(NeedsReview, IncludeInMaintenanceDraft, InvoiceImportFileId);
END;
GO
