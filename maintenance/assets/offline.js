(function () {
  ...
  function register() {
    /*
     * AGILITY: no service worker. The module is served from the signed-in
     * dashboard, where cached authenticated pages would be a liability and
     * every save is meant to reach the server. The banner below still tells
     * the technician what is held on the phone when the connection drops.
     */
    if (!window.WILSON_ENABLE_SERVICE_WORKER) return;
