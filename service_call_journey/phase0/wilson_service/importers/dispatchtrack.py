...
class ImportResult:
    ...
    def summary(self) -> str:
        return (f"{self.file_name}: {self.rows} rows, {self.sv_orders} SV orders -> created {self.created}, updated {self.updated}, "
                f"unchanged {self.unchanged}, attached {self.attached}, dropped {self.dropped_from_feed}, new zones {self.new_zones}, "
...
def import_file(db: DB, path: str, *, now: Optional[_dt.datetime] = None, encoding: Optional[str] = None, file_name: Optional[str] = None) -> ImportResult:
    ...
    try:
        ...
        for sv, grp in groups.items():
            ...
            job_id, outcome = _upsert_order(db, sv, grp, ts)
            seen[sv] = job_id
...
def _upsert_order(db: DB, sv: str, grp: list[dict], ts: str) -> tuple[int, str]:
    h = grp[0]
    epass_status = h.get("Job Status") or None
