...
from .common import (canonical_code, compact_json, ensure_zone, find_or_create_address, find_or_create_customer, parse_date, parse_money,
                     split_name, tech_id_for, upsert_unit, warranty_flags, zone_for_zip, CATEGORY)
...
class ImportResult:
    ...
    def summary(self) -> str:
        return (f"{self.file_name}: {self.rows} tickets -> created {self.created}, updated {self.updated}, unchanged {self.unchanged}, "
                f"attached {self.attached}, new statuses {self.new_statuses}, sync {self.sync}, stale {self.stale}")
...
def import_file(db: DB, path: str, *, now: Optional[_dt.datetime] = None, file_name: Optional[str] = None) -> ImportResult:
    ...
    try:
        ...
        for i, r in enumerate(rows, 1):
            ...
            job_id, outcome, new_status = _upsert_ticket(db, r, ts, now, horizon)
            seen[sv] = job_id
...
def _upsert_ticket(db: DB, r: dict, ts: str, now: _dt.datetime, horizon_days: int) -> tuple[int, str, int]:
    sv = r["Invoice #"]
    ...
    if job is None:
        ...
        return job_id, "created", new_status
    ...
    if attached:
        return job["job_id"], "attached", new_status
    return job["job_id"], ("updated" if substantive else "unchanged"), new_status
