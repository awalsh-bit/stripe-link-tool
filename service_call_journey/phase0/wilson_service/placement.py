...
from .capacity import is_open, blocked_minutes, tech_day
...
def day_load(db: DB, tech_id: int, date: str, exclude_job_id: Optional[int] = None) -> dict:
    ...
    for p in pts:
        if p is not None:
            ...
            cur = p
    drive += _drive(db, cur, SHOP) if stops else 0
    ...
    return {"stops": stops, "points": pts, "work": work, "drive": drive, "capacity": cap, "remaining": cap - work - drive}


def marginal_drive(db: DB, pts: list, here) -> tuple[int, int]:
    """Cheapest insertion of `here` into shop→p1→…→pn→shop. Returns (added minutes, insert index)."""
    if here is None:
        return 0, len(pts)
    route = [SHOP] + [p for p in pts if p is not None] + [SHOP]
    if len(route) == 2:
        return _drive(db, SHOP, here) * 2, 0
    best, at = None, 0
...
def suggest(db: DB, job_id: int, *, earliest: Optional[str] = None, horizon_bd: Optional[int] = None, now: Optional[_dt.datetime] = None,
    ...
    for _ in range(horizon_bd):
        for t in techs:
            ...
            added, at = marginal_drive(db, load["points"], here)
...
def offer(db: DB, job_id: int, now: Optional[_dt.datetime] = None) -> list[dict]:
    """Dates for the picker. With offer.route_first: the cheapest day (within offer.max_defer_days of the earliest) comes
    first and is labelled; a penciled day (SO5 after a pencil) is that day. The earliest open day is always in the list."""
    now = now or _dt.datetime.now()
    job = db.fetchone("SELECT * FROM job WHERE job_id=?", (job_id,))
    if not job:
        raise ValueError(f"job {job_id} not found")
    ...
    max_defer = db.setting("offer.max_defer_days", 5)
    pick = None
    if job.get("penciled_date") and str(job["penciled_date"])[:10] in by_date:
        pick = by_date[str(job["penciled_date"])[:10]]
        pick["label"] = "Best fit — we already have your installer nearby that day"
    elif db.setting("offer.route_first", True):
        within = [c for c in dates if (_dt.date.fromisoformat(c["date"]) - _dt.date.fromisoformat(first_open)).days <= max_defer]
        pick = min(within, key=lambda c: c["cost"]) if within else dates[0]
        pick["label"] = ("Best fit — our route is already in your area that day" if pick["same_zone"] else
                         "Best fit — closest to our route that day") if pick["date"] != first_open else "Earliest available"
    ordered = ([pick] if pick else []) + [c for c in dates if pick is None or c["date"] != pick["date"]]
    n = db.setting("booking.max_offers", 7)
    out = ordered[:n]
    if not any(c["date"] == first_open for c in out):
        out.append(dates[0])
    for i, c in enumerate(out):
        c["recommended"] = (i == 0)
        c.setdefault("label", "Earliest available" if c["date"] == first_open else "")
    return out
