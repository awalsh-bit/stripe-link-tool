...
KINDS = ("create_ticket", "set_status", "set_schedule", "add_lines", "post_payment", "close", "note")
...
def render_packet(sv_number: Optional[str], customer_name: str, kind: str, p: dict) -> str:
    ...
    if p.get("lines"):
        ...
        for ln in p["lines"]:
            ...
            lines.append(f"  {code:<12} {ln.get('desc', '')}  x{ln.get('qty', 1)}  ${float(ln.get('price') or 0):,.2f}")
    if p.get("payment"):
        pay = p["payment"]
