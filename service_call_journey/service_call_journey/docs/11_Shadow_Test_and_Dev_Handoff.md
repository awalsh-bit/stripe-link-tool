...
> **Superseded on transport ⟨9/15⟩.** `12_Agility_Platform_Notes.md` confirms this runs inside Agility (Node + Postgres on Render), not as a Python process beside ePASS: the shim in §3–§4 becomes routes under `/api/service-journey/`, the DispatchTrack feed already arrives by agent push, and the copy button posts `{serviceCardId}` for the server to read from `service-cards.json`. The plan, the field mapping and the morning check below are unchanged in substance — see `13_Agility_Alignment.md` for the mapping and for what the Agility port still needs.
