...
| **SO5** — parts in | The install goes to the **owning tech only**: date cleared, then placed on the owner's first day with capacity on/after the parts ETA (or the next open day), as a pencil the dispatcher confirms. Owner is the lane (John) or office-mode → stays in the lane / Unscheduled. No owner known → Unscheduled "Needs a date". | Owner's column · packet in Sync queue |
