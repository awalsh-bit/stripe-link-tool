...
class DB:
    ...
    def insert_many(self, table: str, cols: list, rows: Iterable[Iterable[Any]], chunk: int = 5000) -> int:
        """Bulk insert for the one-off history loads (§1.4, §1.6). Row-at-a-time `insert` costs a
        round trip each; the full ePASS export is ~830,000 rows, which is minutes rather than seconds
        without this."""
        sql = f"INSERT INTO {table} ({', '.join(cols)}) VALUES ({', '.join('?' for _ in cols)})"
        n, buf = 0, []
        for r in rows:
            buf.append(tuple(_coerce(v) for v in r))
            if len(buf) >= chunk:
