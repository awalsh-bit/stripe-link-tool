...
# ------------------------------------------------------------------ 9/15: ePASS's parking date
DOW = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]


def any_work_days(db: DB) -> set:
    """Weekday names at least one active tech works. Wilson runs Mon–Fri, so a Saturday date is nobody's route."""
    out = set()
    default = db.setting("tech.work_days_default", "Mon,Tue,Wed,Thu,Fri")
    for t in db.fetchall("SELECT work_days FROM tech WHERE active=1"):
        for d in (t.get("work_days") or default).split(","):
            if d.strip():
                out.add(d.strip())
    return out or set(d.strip() for d in default.split(","))


def is_parking_day(db: DB, date: Optional[str], workdays: set) -> bool:
    """ePASS parks work with no real date on the coming Saturday (9/10 export: Sat 9/12 ×121; 9/15: Sat 9/19 ×155).
    Nobody routes that day, so such a date means 'unscheduled', not 'booked'."""
    if not date or not db.setting("import.parking_day_rule", True):
        return False
    try:
        return DOW[_dt.date.fromisoformat(date).weekday()] not in workdays
    except ValueError:
        return False
