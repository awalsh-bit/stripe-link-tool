...
def _verified_target(job, ctx):
    ...
    # ⟨9/19⟩ a cosmetic-damage repair has no estimate stage at all: we damaged it, we pay for it, and the person who
    # approves the part is the service manager rather than the customer. So it goes straight to "order the part".
    if ctx.get("quote_kind") == "damage":
        return "SO3"
    return "SO2.1"
...
def _run_effect(db: DB, eff: str, job: dict, ctx: dict, ts: str, frm: str, to: str) -> None:
    ...
    if eff == "set_owner":
        ...
        return
    if eff == "record_visit":
        from . import findings as _findings   # ⟨9/19 late⟩ the visit is a row in history, not just an event
        _findings.record(db, job, dict(ctx, _to=to), ts)
...
def create_request(db: DB, *, customer: dict, address: dict, unit: Optional[dict] = None, problem_text: str = "", card_saved: bool = False,
    ...
    # ⟨9/18 late⟩ the private tracker link exists from the moment the request does, because the confirmation
    # page and the request_received text both print it (§6b)
    from . import tracker
    token = tracker.issue(db, job_id, by=actor_id, now=now)
    job = db.fetchone("SELECT * FROM job WHERE job_id=?", (job_id,))
    from . import kpi
    kpi.detect_recall(db, job_id, now)
    _outbox(db, job, "notify:request_received", {"status": status, "link": tracker.BASE + token}, ts)
